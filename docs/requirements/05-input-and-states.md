...
- **Launch branching (no counter decrement):**
  ...
  - If `!isFreeShotActive && supply.freeShot>0 && getAttemptsLeft()<=1` (legacy auto-arm kept for safety, but primary Free Shot arming now also occurs via banner logic after counter decreases to 1) then `isFreeShotActive=true` and `syncFreeShotGlow()`. No manual hotkey; `4` is `magnifier` (legacy `rotate` was `4`; new order `1 Liquifier, 2 Deflector, 3 Rotator, 4 Magnifier`).
