...
<body>
  <div id="game-container">
    ...
    <div id="hud">...</div>                                     <!-- z-index:4 HTML header, pointer-events:none -->
    <div id="hotbar">...</div>
...
- Inside `#game-container` stacking via CSS `canvas,#wind-canvas { position:absolute; inset:0; width:100%; height:100% }` with `#bg-canvas{z-index:1}` `#game{z-index:2}` `#wind-canvas{z-index:3; pointer-events:none; background:transparent}` `#hud{z-index:4; pointer-events:none;}` and `pointer-events:none` on `bg`+`wind`+`hud`; `game` is the only input target.
- Overlays (`#hud`,`#hotbar`,`#reward-overlay`,`#win-overlay`,`#main-menu-overlay`,`#help-overlay`,`#toast`) are `position:absolute; inset:0` or anchored to container edges, `z-index:4-12`, bounded to container (`getBoundingClientRect()` inside container). Scrollable children use `overflow-y:auto; overscroll-behavior:contain`.
