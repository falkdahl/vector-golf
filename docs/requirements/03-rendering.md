...
- **Aim visuals** (see `05-input-and-states.md` §2): orbit `28-32px` dashed `rgba(0,0,0,0.2)`, aim line `30px` (+ `charge*50`), indicator dot. Drawn only when `gameState ∈ {'AIMING','CHARGING'}` and not while `rewardMenuVisible`/`coinSummaryVisible`/cutscene/banter active or a hole transition (`pendingHoleAdvance`) is pending — so the orbit is never drawn around the ball sitting in a cleared hole (e.g. tutorial hole 1 before the `first-restock` cutscene).
...
- **Hole & Attempts Banners** (`src/render.js:drawCenterBanner`, see `05-input-and-states.md` §6): transient dim `rgba(0,0,0,0.55)` `700 22px white stroke 5px` `1000ms`; `Hole N` before reward, `Last Attempt` when `attemptsLeft===1 && freeShot===0` after counter decreased, `Free Shot!` when `attemptsLeft===1 && freeShot>0` after counter decreased. Mutually exclusive with reward overlay `#reward-overlay`. Both block input.
