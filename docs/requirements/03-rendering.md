...
- **HUD** `drawHUD(ctx, currentHoleIndex, totalHoles, holeAttempts, totalAttempts, maxAttempts, freeShotSupply)` every `render()`: `Hole: N/M` at `(12,22)` left, **`Attempts Left: X`** (where `X = max(0, maxAttempts - holeAttempts)`, **not** `Attempts: X`) **plus optional free-shot stock** ` (+Y)` **only when `Y = supply.freeShot > 0`**, e.g. `Attempts Left: 5 (+5)` when `supply.freeShot=5`, and `Attempts Left: 5` (without parentheses) when `supply.freeShot===0` — HUD **must not** show `(+0)` when no free shots, must show `(+Y)` with integer `Y` when `Y>0` at `(W/2,22)` center, `Total: Y` at `(W-12,22)` right, `14px system-ui`, fill `white`, stroke `rgba(0,0,0,0.7) 3px` or shadow, semi-transparent strip `rgba(0,0,0,0.25) 28px` behind. `maxAttempts` starts `10` and is **static** (no `Max Attempts +5` reward; superseded by `Free Shot` per `07`/`09`). `supply.rotate` starts `1` (not shown in HUD, only in hotbar). HUD visible in `AIMING`/`CHARGING`/`FLYING`, also dimmed behind `WIN`/`GAME_OVER`. When `isFreeShotActive===true`, HUD may optionally show `Free Shot armed` hint near ball, but `Attempts Left` value itself is unchanged until launch; the `(+Y)` is shown **only if `supply.freeShot>0`**.
...
- **Modifier circles & preview** (spatial `amplify`/`nullify`/`flip`/`rotate`): see `07-modifiers.md` (rotate **red** `↻` `rgba(231,76,60,0.25)` with **red icon** `↻` `#e74c3c`); **Free Shot gold glow** is rendered in Three.js layer over ball, not on `game` canvas.
...
- **Hole & Attempts Banners** (`src/render.js:drawCenterBanner` / `drawHoleBanner` / `drawAttemptsBanner`, see `11-banners.md`): transient center banners with same backdrop as reward menu (`rgba(0,0,0,0.55)` full-dim, `700 22px white stroke 5px` centered) on `fgCtx`, shown `1000ms` then auto-hide. Hole banner `Hole N` at start of each hole before reward (1s); attempts banner `Last Attempt` before last counted attempt **only when `supply.freeShot === 0`** (suppressed while free shots remain). Rendered after `drawHUD`/`drawForceBar`, mutually exclusive with `drawRewardMenu` (hole banner → reward transition, attempts banner auto-hide). Block launch/placement while visible, wind still animates.
