...
- **Active-run**: `STORAGE_KEY="golfVectorField.progress.v1"` JSON `version:1` payload:
  ...
  { version:1, courseId:string, currentHoleIndex:number, holeAttempts:number, totalAttempts:number,
    supply:{amplify, nullify, flip, rotate, freeShot}, maxAttempts:number, holeAttempts:number, attemptsLeft:number, areaUpgradeCount:number,
    isFreeShotActive:boolean, // whether next launch is free (auto-armed when attemptsLeft<=1 && supply.freeShot>0, golden glow, no manual 4)
  ...
  ```
  Transient `ball.pos/vel/isMoving/z/vz`, `gameState==="FLYING"` (resume always `AIMING` at tee unless `GAME_OVER` — see §7), `charging/charge`, `mousePos`, field grid not persisted; on resume ball at tee `vel=0` unless loading `GAME_OVER` (then show Game Over screen). Treasure `isCollected` restored for current hole; other holes' treasures are re-generated via `level.treasure` from `COURSES_KEY` and `isCollected` for future holes is `false`. All numbers clamped `≥0` on load; `maxAttempts` defaults `10` if missing (legacy `maxAttempts>10` kept but no reward increases); `supply.freeShot` defaults `0` (passive stock shown as `(+Y)` after `Attempts Left`) and `supply.rotate` defaults `1` if missing (new games start `{1,1,1,1,0}` with `rotate:1`); `isFreeShotActive` defaults `false` (auto-only, no manual `4`); `treasure` missing → regenerate via `generateTreasureForHole` (see `08` §4) as uncollected; `holeAttempts` defaults `0`; `attemptsLeft = max(0, maxAttempts - holeAttempts)` recomputed, HUD shows `Attempts Left: X (+Y)` where `Y=supply.freeShot`. Missing fields default `0/false/[]`; corrupt/`version!==1`/`courseId` missing in courses → treat as no save. Wrap in `try/catch`; quota/error fallback to new game.
...
- `handleLaunch` (free launches do **not** decrement `attemptsLeft` but still call `saveProgress()` after consuming `supply.freeShot` per `07` §5.2 via auto-arm when `attemptsLeft<=1`; counted launches decrement `attemptsLeft` and HUD shows `Attempts Left: X (+Y)`) calls `saveProgress()`; also after `claimReward` (`Free Shot Supply +5` increases `supply.freeShot` shown as `(+Y)` or `Rotate +1` adds `supply.rotate`), `rerollReward` (costs 1 counted attempt toward `maxAttempts`, never free), `placeModifier`/removal/drag (including `rotate` via `4`), `collectTreasure` (`treasure.isCollected=true` + `rewardPending` per `09` §3, see `04` §6), `advanceHole`/`handleNextHole`/`loadLevel` (including hole-start `rewardPending` for holes >0 and fresh `treasure` uncollected), `maybeShowRewardMenu` when creating fresh `rewardOffered`, and on `GAME_OVER` trigger. `toggleFreeShot` via `4` is removed (free shot has no hotkey; `4` is now `rotate`).
...
- Auto-restore: resolve `activeCourse = courses.find(c=>c.id===data.courseId)`, `LEVELS=activeCourse.holes`, restore all persisted fields (`maxAttempts`, `holeAttempts`, `attemptsLeft` recomputed, `supply` with `rotate` and `freeShot` (`rotate` defaults `1` if missing, `freeShot` `0`), `isFreeShotActive` via auto-arm only, `treasure.isCollected` for current hole), recompute `areaMultiplier` (no `bouncyRemaining` — trees always bounce), recreate field `createField(cols,rows,strength,seed,W,H)` then `setModifiers(modifiers)` (including `rotate` circles `↻` teal), restore `level.treasure.isCollected` from saved `treasure` (or regenerate if missing) and hidden/visible accordingly, restore golden glow if `isFreeShotActive` (passive, `(+Y)` in HUD), ball at tee `AIMING` (never `FLYING`/`WIN`) **unless** saved `gameState==='GAME_OVER'` — then set `gameState='GAME_OVER'`, show Game Over overlay (see §7) instead of `AIMING`, still hide main-menu overlay but bottom shows grass dimmed. If saved `rewardMenuVisible` and not Game Over, show reward menu. If saved `rewardPending` was true due to hole-start or treasure hit, `maybeShowRewardMenu` will show it after restore when `AIMING` (or immediately even in `FLYING` for treasure). If no save/corrupt, stay on main menu. `loadProgress()`/`hasRestorableSave()` handle validation; `maxAttempts` missing → `10`; `supply.freeShot` missing → `0` (HUD `(+0)`), `supply.rotate` missing → `1`; `isFreeShotActive` missing → `false` (auto-only); `treasure` missing → regenerated uncollected.

### Clear on abandon / new course start / Game Over

- `clearProgress() => localStorage.removeItem(STORAGE_KEY)` and reset run state to `currentHoleIndex=0, holeAttempts=0, totalAttempts=0, maxAttempts=10, supply={1,1,1,1,0}` (`amplify:1, nullify:1, flip:1, rotate:1, freeShot:0`), `isFreeShotActive=false` (passive, no hotkey), treasure uncollected for hole 1 (regenerated via `generateTreasureForHole`), `areaUpgradeCount=0`, `rewardPending=false`, `rewardOffered=[]`, `modifiers=[]` (including `rotate` teal)… and clear gold glow + hide treasure collected flag, without touching `COURSES_KEY`. Called on `resetGameAfterWin` (`R` in `WIN`/`GAME_COMPLETE`), `startNewGameFromMain` (course play), `endRun` (see §6) **and on `Game Over → Return to Main Menu`** (see §7). Only **full completion** updates `bestTotal` (see §5), `End Run` and `Game Over` do not. New games start with `1` `rotate` and `0` `freeShot` (HUD `Attempts Left: 10 (+0)`) and fresh uncollected treasure per hole.
...
- **Stages list** `#staged-course-list` (inside `#main-menu-root`) bounded to canvas (`max-height: min(42vh,320px)`, `overflow-y:auto`, `display:flex; gap:8px`, `flex-direction:column`, `max-width:480px` so that course name + record text is fully visible without truncation, `.course-play-button` `min-width:260px` `padding:10px 16px` `overflow:visible`, `.course-name`/`.course-meta` `white-space:nowrap` `overflow:visible` `text-overflow:clip`). For each `holeCount` in `STAGES=[3,6,9,18]` in order:
  ...
  - If **unlocked**: row `.course-row[data-course-id][data-holes="3"]` with:
     ...
     - Clicking the **play button** (not refresh) starts new run on that stage's course: same as before `activeCourse=course; LEVELS=course.holes; currentHoleIndex=0; holeAttempts=0; totalAttempts=0; maxAttempts=10; supply={1,1,1,1,0}` (`rotate:1`); `isFreeShotActive=false` (passive); `loadLevel(0); gameState="AIMING"; mainMenuVisible=false; saveProgress(courseId)`.
...
- `function endRun(){ clearProgress(); currentHoleIndex=0; holeAttempts=0; totalAttempts=0; maxAttempts=10; supply={1,1,1,1,0}` (`rotate:1`); `isFreeShotActive=false` (passive); `areaUpgradeCount=0; ...; modifiers=[]; syncModifiersToField(); clearFreeShotGlow(); mainMenuVisible=true; isInLevelPause=false; courseMenuVisible=false; helpVisible=false; /* no maybeUpdateHighScore */ }`
- Removes `STORAGE_KEY` only; preserves `COURSES_KEY`; `bestTotal` unchanged.
 - **Game Over** (`gameState='GAME_OVER'` when **last counted attempt fails** after `attemptsLeft` became `0` — `terrainHit`/`waterHit`/`edgeOut` in `update()` while `FLYING` with `attemptsLeft<=0` and no free-shot win; fallback `handleLaunch` while `attemptsLeft<=0` also triggers): the last counted shot where `attemptsLeft` becomes `0` is **allowed to finish** (`FLYING` continues, `checkWin` can still win); its **failure** (OB/water/edge) shows Game Over **immediately without `resetBall()` to tee** (see `05` §5). Free launches do not count and last-attempt auto-arm (`supply.freeShot>0` at `getAttemptsLeft()<=1` makes launch free via passive `Attempts Left: X (+Y)`, see `05` §4 & `07` §5.2, no hotkey) can prevent `attemptsLeft` from reaching `0`. Once `GAME_OVER` is entered: full-canvas dim `rgba(0,0,0,0.55)`, big title `Game Over` `700 22px` white `stroke 5px` centered **only** over green `Continue` button (`#gameover-return-button` `background:#145a32` text `Continue`) with `Press R to continue` hint (`win-hint`). While `GAME_OVER`, `updateBall`/`handleLaunch`/modifiers (`1`/`2`/`3`/`4` now `amplify`/`nullify`/`flip`/`rotate`)/pause are blocked; only green `Continue` (click or `R`, also `Escape`/`Enter` for compat) is accepted. It does `clearProgress()` removing active game (`STORAGE_KEY`) (same reset as `endRun` with `maxAttempts=10`, `supply {1,1,1,1,0}`, `isFreeShotActive=false` passive) and returns to entry main menu (`mainMenuVisible=true`, `isInLevelPause=false`, `gameState='AIMING'` after clear). No other button from Game Over; reload after Game Over shows entry with no active game. Also not saved as win — `bestTotal` unchanged. **Pressing `R` on either `Course Completed!` Victory or `Game Over` takes to main menu and removes current active game.** Free Shot **persists** while supply remains after a free launch, so consecutive free launches are supported without re-arming (auto only).
...
- **Content** (`src/help`): Must show, in order, the sacred-links lore verbatim (or semantically identical) followed by the control scheme. Lore text normative:
  ...
  ```
  Rules keywords still satisfied: `wind`+`hole`/`course`+`attempt` (`wind` appears in lore, `hole`/`course` in “every hole must be aced…anywhere on the course”, `10 attempts`); controls must contain `arrow`/`aim` + `space`/`charge`/`shoot` + `click`/`place`; plus modifiers-before-shoot, `Rotate` (`4` `↻` teal) and `Free Shot` passive `Attempts Left: X (+Y)` gold glow (no hotkey) mentioned in controls, per-course `bestTotal` saved via lore context. Controls list rows `Arrow keys — Aim`, `Space — Hold to charge, release to shoot`, `Click — Place modifier`, `Right-click — Remove`, `1 / 2 / 3 / 4 — Select Amplify / Nullify / Flip / Rotate`, `Attempts Left (+Y)` — free shots shown after attempts, `H`, `R`, `Escape/P` pause (core three `Arrow/Space/Click` mandatory) must follow the lore in the same `.help-card`.
