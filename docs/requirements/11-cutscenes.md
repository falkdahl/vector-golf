...
- **Loadout consequence:** The loadout shown immediately after `intro-3-hole` (i.e., the loadout triggered by the prologue → intro-3-hole chain for the 3-hole course) shall **not contain any shop panel** (see `10-progression.md` §2.2 shop hidden when intro-3-hole just played). Subsequent loadouts (after `intro-3-hole` is already seen) shall show shop normally.
...
- `index.html:1` (`#cutscene-dialog` overlay with `.cutscene-portrait`, plus `#cutscene-title-overlay`/`#cutscene-fade-overlay` black + title)
- `style.css:1` (JRPG dialog box + portrait, editor styles if shared, plus `#cutscene-title-overlay` / `.cutscene-title-text` fade styles)
