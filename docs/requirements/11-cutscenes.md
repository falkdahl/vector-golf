...
## 11c. First-Restock Cutscene — Before Loadout on First Restocked Run (Guaranteed on Second Run)

- The cutscene `src/cutscenes/first-restock.json` (`id: "first-restock"`) shall be presented exactly once, on the start of the run where the shop is restocked for the first time — i.e. the first `handleCoursePlay(courseId)` (any unlocked course) after the first shop restock has occurred — before the loadout overlay is shown. **Since `10-progression.md` §1.6 guarantees the first restock at the start of the second run regardless of clear, this cutscene is guaranteed to play on the second `handleCoursePlay` (second run) even if the player did not clear the 3-hole course.** Before that second run, `handleCoursePlay` ensures the restock via `ensureFirstRestockOnSecondRun()` ( `runsStarted >=1 && !isShopRestockedFirstTime()` → `grantShopMilestoneStock(3)` ), so `isShopRestockedFirstTime()` becomes true in time for the trigger on that same call.
- **Restocked definition:** the shop counts as restocked when `getShopStock()` has any count `>0` (first milestone granted per `10-progression.md` §1.6, either via 3-hole clear or via the guaranteed second-run grant) or, equivalently when the shop was already emptied by purchases, when the 3-hole course has been cleared (`courses.find(c=>c.holeCount===3).bestTotal !== null`, which is what grants the first restock: deflector + rotator + magnifier) **or** when `runsStarted >=1` and the second-run guarantee has fired (shop now has `deflector/rotator/magnifier`). Either condition means the first restock has happened. Helper `isShopRestockedFirstTime()` (or inline check) shall return true in any of those cases.
- **Trigger:** `handleCoursePlay(courseId)` with `hasRestorableSave()===false` and `!cutsceneIsActive()` and `!hasSeenCutscene('first-restock')` and shop restocked per above (after the `ensureFirstRestockOnSecondRun()` pre-check) shall intercept before `showLoadout(courseId)`: hide main menu/splash/parallax immediately (same as prologue §11), `loadCutscene('first-restock')`, then `playCutsceneWrapped(data, {onComplete})` → on complete `markCutsceneSeen('first-restock')` + `ensureFirstRestockStock()` (if stock for `deflector`/`rotator`/`magnifier` still not present, add it) → then `showLoadout(courseId)` (shop visible normally, now guaranteed with `deflector`/`rotator`/`magnifier` in stock directly after cutscene). Direct `startCourseWithLoadout` without going through `handleCoursePlay` shall not trigger it. If the JSON fails to load/validate or play fails, fall back to `showLoadout` without blocking but still `ensureFirstRestockStock()` before show (marking seen to avoid loops on invalid data, same as prologue fallback).
- **Ordering:** prologue/intro-3-hole chain (§11/§11b) takes precedence when due (first 3-hole play, shop still empty so no conflict in practice — first run is always prologue, not restock). `first-restock` is checked after the prologue branch: if prologue was due it chains to loadout directly; otherwise if `first-restock` is due (which on second run is guaranteed after the `ensureFirstRestockOnSecondRun()` grant) it plays before loadout. While active, `syncMainMenu`/`syncParallaxVisibility`/`redrawBottom`/`syncProgressionDisplay` shall stay hidden exactly as for prologue so the cutscene background is visible.
- **Persistence:** same `CUTSCENE_SEEN_KEY` array as prologue. Once `seen` contains `"first-restock"`, subsequent run starts go directly to `showLoadout`. `localStorage.clear()` / `regenerateCampaign()` clears seen, so a new campaign shows it again after its first restock (which again is guaranteed on second run).
