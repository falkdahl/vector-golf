...
- **Easy angle limit:** for `tier==='easy'` (`I`-shaped) the vertical angle between tee and hole shall satisfy `|atan2(hole.y-tee.y, hole.x-tee.x)| ≤ 10°` (`0°` is same `y`). If sampled `|Δy|` exceeds `Δx * tan(10°) ≈ Δx*0.176`, clamp `hole.y` to `tee.y ± Δx*tan(10°)` (nearest bound) while keeping `hole.y∈[80,LOGICAL_H-80]` and deterministic for same seed. This prevents overly long tilted easy holes.
...
- [ ] **Easy angle `|atan2(Δy,Δx)| ≤10°`** for all `easy` holes (updated from 20° to 10°, `tan 10°≈0.176`).
