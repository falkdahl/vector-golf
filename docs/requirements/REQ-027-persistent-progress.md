...
## Description
The game SHALL persist run progress to `localStorage` **on each attempt** (every `handleLaunch` that counts as a shot) so that when the site is revisited (page reload, tab closed/reopened, browser restart) the game **can be resumed via the main menu "Continue" button** (REQ-029) — the site SHALL show the main menu with "Continue" visible when a restorable save exists, and clicking "Continue" restores the saved state. No auto-resume without user interaction; the save is loaded on demand via "Continue".
...
3. **Load & Resume on Revisit — Manual via Continue (Not Auto-Resume)** in `src/main.js:init()` / `src/main.js:setup`:
   - On page load, `init()` SHALL evaluate but **SHALL NOT auto-resume** into the game — it SHALL show the **main menu root** (REQ-029) and conditionally show the **"Continue"** button if `hasRestorableSave()` is true. Actual restoration of the run state happens **only when the user clicks "Continue"** (see REQ-029 §3). `loadProgress()` is still the deserializer but is called on demand by the Continue handler (and on load only to decide button visibility). The logic for `loadProgress()` remains:
     ```js
     ...
     ```
    - `hasRestorableSave()` SHALL be:
      ```js
      function hasRestorableSave(){
        try{ const raw=localStorage.getItem(STORAGE_KEY); if(!raw) return false;
          const d=JSON.parse(raw); if(!d||d.version!==1||!d.courseId) return false;
          return !!loadCourses().find(c=>c.id===d.courseId);
        }catch{ return false; }
      }
      ```
      `init()` SHALL call `hasRestorableSave()` to toggle `#continue-button` visibility (REQ-029 §3) but SHALL NOT restore state yet; it SHALL keep `mainMenuVisible=true` with bottom splash.
    - If the user clicks **"Continue"** and `loadProgress()` returns valid data and `data.courseId` exists in the courses collection (`loadCourses().find(c=>c.id===data.courseId)`) and `data.currentHoleIndex` is within `0..course.holes.length-1` (where `LEVELS` is now `course.holes` of the active course per REQ-031):
      ...
      - Set `mainMenuVisible=false`, hide `#main-menu-overlay`, switch bottom to grass via `drawBackground('grass')`, `updateAttemptsUI()`/`updateHotbarUI()`/`drawHUD` on next frame so restored HUD `Hole: N/M Attempts: X Total: Y` is visible without an extra launch.
    - If no saved data, corrupt JSON, wrong version, or out-of-range indices, clicking Continue does nothing (or hides it after re-check); the main menu stays on root and a new game can be started via New Game → course list. An error in `loadProgress` SHALL NOT throw or block rendering. If `hasRestorableSave()` is false on load, `#continue-button` stays hidden.
...
4. **Clear on New Game / End Run (Abandon Without Record)** in `src/main.js:resetGameAfterWin()` / `clearProgress()` / `startNewGameFromMain()` / `endRun()`:
   - Pressing `R` in `WIN` / `GAME_COMPLETE` (reset entire run per REQ-011/REQ-014), `startNewGameFromMain` (REQ-029 main menu New Game → course pick), or `endRun` (REQ-028/029 Escape pause → `End Run`) SHALL call `clearProgress()` which does `localStorage.removeItem(STORAGE_KEY)` and then re-initializes all run state to new-game defaults (`currentHoleIndex=0`, `holeAttempts=0`, `totalAttempts=0`, `supply={1,1,1}`, `freeShots=0`, `areaUpgradeCount=0`, `bouncyBallCount=0`, `sharpshooterCount=0`, `secretRewardCounter=0`, `rewardPending=false`, `rewardOffered=[]`, `modifiers=[]`, `rewardRerolled=false`). The next attempt SHALL then trigger a fresh save.
   - **`End Run` shall NOT count toward record:** `endRun` (triggered via `End Run` button in the Escape pause menu with backdrop, which simply hides the overlay and shows entry menu, or via legacy `End Run` path) SHALL **not** call `maybeUpdateHighScore()` / `maybeUpdateCourseRecord()`; the abandoned run's `totalAttempts` SHALL be discarded and `bestTotal` for the course SHALL remain unchanged (`null` if never completed, or previous best if it existed). Only a **full course completion** (`WIN` on final hole, `currentHoleIndex === course.holes.length-1`, `gameState==="WIN"`, via `Next`/`Continue` on win overlay) SHALL update `bestTotal` per REQ-031.
   - Optional: exposing `window.__clearProgress()` / `window.clearProgress()` for tests. Reloading the page SHALL NOT clear storage; only explicit new-game reset / End Run does.
...
- [ ] On fresh play (no key), `localStorage.getItem("golfVectorField.progress.v1")` is `null`; starting a new game shows **no reward menu** (`rewardMenuVisible===false`) and `supply={1,1,1}`; no save is created until an attempt is made (still `null` before first launch).
 - [ ] After a counted launch (`freeShots=0` → `holeAttempts 0→1, totalAttempts 0→1, secretRewardCounter 0→1`), `localStorage` now contains a JSON string with `version:1`, `currentHoleIndex:0`, `holeAttempts:1`, `totalAttempts:1`, `supply` as at that moment (`{1,1,1}` plus any claimed), `savedAt` within 2s of now. Reloading the page shows main menu with **"Continue" visible** (not auto-resume); clicking "Continue" restores HUD to `Hole:1/M Attempts:1 Total:1` (not `0/0`), and `getSecretRewardCounter()===1`.
 - [ ] After a free-shot launch (`freeShots=2` → `1`, counters unchanged `Total 5→5`), `localStorage` is updated with `freeShots:1` while `totalAttempts` stays `5` and `secretRewardCounter` unchanged. Reloading shows Continue; clicking Continue restores `freeShots 1` and `Total 5`.
 - [ ] After claiming a reward after 5 counted shots (`claimReward('amplify')` when offered → `supply.amplify 1→2`), storage is updated with the new supply even before the next attempt. Reloading + Continue restores the same supply and hotbar shows `2` capability.
 - [ ] After placing an Amplify modifier with supply, `modifiers.length 0→1` is persisted. Reloading + Continue restores the modifier at the same `x,y,radius` (effective radius reflects `areaUpgradeCount`), and arrows inside the circle show amplified wind via `getWindAt`.
 - [ ] Multi-hole: clearing hole 1 (`advanceHole()` → `currentHoleIndex 0→1, holeAttempts 0, totalAttempts preserved`) saves with `currentHoleIndex:1`, `secretRewardCounter` reset to `0` and `rewardPending=true` so reloading shows main menu with Continue; clicking Continue shows `Hole:2/M` with a reward menu pending before first attempt on hole 2 (`rewardMenuVisible` true after Continue if `rewardOffered` present). `Attempts:0` for that hole, `Total` as before, with previous `supply`/`freeShots`/`area`/`bouncy` all preserved.
 - [ ] Reroll: with menu visible after 5 counted shots and `rewardRerolled=false`, triggering `rerollReward()` → `Total 5→6` cost, new `rewardOffered` 3-set, `rewardRerolled true`) persists `totalAttempts:6` and the new offer and `rewardRerolled:true`. Reloading + Continue still shows the **re-rolled** menu (same 3 new options) and the reroll button disabled, not the original offer.
