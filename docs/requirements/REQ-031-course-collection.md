...
1. **Course Data Model** in `src/courses.js` or `src/levels.js`:
   - A `Course` SHALL be:
     ```js
     {
       ...
       holes: Level[],      // 3, 6, 9 or 18 items, each as per REQ-010 Level shape
       holeCount: 3|6|9|18,   // holes.length
   ...
   - `holes` SHALL be generated via `generateLevels(seed, holeCount, {difficulty})` (variable count, REQ-034/010). `holeCount` MUST be `3`, `6`, `9` or `18`. For `3`-hole courses, `options.difficulty` SHALL be one of `'easy'|'medium'|'hard'` selected by the player (see §3 New Course flow) and all holes SHALL be generated with `tier === difficulty`. For `6`, `9` or `18`-hole courses, difficulty SHALL scale linearly: hole 1 `easy`, last hole `hard` (see REQ-034/010 linear mapping).
...
2. **Courses Collection Persistence** in `src/storage.js` / `src/courses.js` / `src/main.js`:
   ...
   - **Load** (`loadCourses()`): on `init()` before rendering menus, attempt `localStorage.getItem(COURSES_KEY)` → `JSON.parse` → validate `version===1` and `Array.isArray(courses)`. On corrupt/missing/wrong version, fallback to auto-created default course and overwrite. Each loaded course SHALL be validated: `id` string, `name` string, `holes` array length `3|6|9|18`, each hole with `tee/hole/obstacles/field` per REQ-010; invalid courses SHALL be discarded (with `console.warn`) and not crash.
...
3. **Course Submenu — Scrollable Course List + Per-Course Actions (Behind New Game)** in `index.html` / `style.css` / `src/main.js`:
   ...
   - **Footer actions** inside course submenu (below the scrollable list, inside `#course-menu` but outside `#course-list`):
    ...
    - **New Course flow**: clicking `New Course` SHALL show a **choice UI** for hole count: `<div id="new-course-choices"><button data-holes="3">3 Holes</button><button data-holes="6">6 Holes</button><button data-holes="9">9 Holes</button><button data-holes="18">18 Holes</button><button id="new-course-cancel">Cancel</button></div>` — must be HTML buttons (not `prompt`). Selecting `6`/`9`/`18` SHALL generate a new course via `createCourse(holeCount)` with linearly scaled difficulty (see REQ-034/010: hole 1 `easy` → last hole `hard`), push to `courses`, `saveCourses()`, re-render the list (now includes new course), and keep course submenu visible (not auto-starting). Selecting `3` SHALL show a **secondary difficulty chooser** `<div id="new-course-choices-difficulty" data-holes="3"><button data-difficulty="easy">Easy</button><button data-difficulty="medium">Medium</button><button data-difficulty="hard">Hard</button><button id="new-course-difficulty-cancel">Cancel</button></div>` (or reuse `#new-course-choices` with difficulty buttons). Choosing `easy`/`medium`/`hard` SHALL generate a new course via `createCourse(3, {difficulty})` where **all 3 holes match the selected difficulty** (`level.difficulty.tier` identical for all holes, see REQ-034/010), push/save/render as above. `Cancel` SHALL hide the choice UI(s) and restore footer.
...
- [ ] Clicking a course's **play button** (not delete) starts a new game on that course: `mainMenuVisible false`, `currentHoleIndex 0`, `holeAttempts 0`, `totalAttempts 0`, `activeCourse.id === clicked course.id`, `LEVELS` length matches `holeCount` (3/6/9/18), `loadLevel(0)` uses that course's tee/hole, bottom canvas shows grass, no initial reward menu, and `STORAGE_KEY` now contains `courseId` equal to the clicked course's `id`. Reloading shows main menu root with Continue visible (not auto-resume) and resumes via Continue.
...
- [ ] **New Course**: in course submenu footer, button `New Course` (`#new-course-button`) is visible. Clicking it shows a choice UI with **four** buttons for `3`, `6`, `9`, `18` holes (e.g., `#new-course-choices` with `data-holes="3"` etc., `data-holes="6"` added) and a `Cancel`. Selecting `6` creates a new course with `holes.length===6` linearly scaled (hole 1 `easy` → 6 `hard`, see REQ-034/010), `9` creates `holes.length===9` linearly scaled (1 `easy` → 9 `hard`), `18` creates `18` linearly scaled (1 `easy` → 18 `hard`). Selecting `3` shows a **secondary difficulty chooser** (`#new-course-choices-difficulty` with `data-difficulty="easy"|"medium"|"hard"`), choosing `easy` creates `holes.length===3` where **all 3 holes are `easy`**, `medium` all `medium`, `hard` all `hard` (uniform, see REQ-034). All creations use random `id`/`name` (adjective+noun, different from existing), push to `courses`, persist, and the list now has `length+1` rows. `Cancel` hides the choice UI(s) without creating a course.
...
- Implementation sketch `src/courses.js`:
  ...
  export function generateCourse(holeCount=18, seed=Date.now(), options={}){
    ...
    const opts = options || {};
    // For 3-hole courses, options.difficulty = 'easy'|'medium'|'hard' → uniform; for 6/9/18 linear scaling inside generateLevels
    const holes = (holeCount===3 && opts.difficulty) ? generateLevels(seed, holeCount, {difficulty: opts.difficulty}) : generateLevels(seed, holeCount);
  ...
  export function importCourse(b64){
    ...
    if(!c.id||!c.name||!Array.isArray(c.holes)||![3,6,9,18].includes(c.holes.length)) throw new Error("Invalid course data");
    return c;
  }
  ```
