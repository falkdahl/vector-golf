...
- **Active-run:** `STORAGE_KEY="golfVectorField.progress.v1"` JSON `version:1` payload:
  ...
  ```
  Legacy `areaUpgradeCount` kept for compat (migrated to `fieldExtenderCount` if present); if both missing default `0`. `powerCellCount` defaults `0` (alias `rangeModifier`). `totalPoints` defaults `0` (new points tracker, see `10-progression.md` §3). Transient `ball.pos/vel`, `FLYING` (resume always `AIMING` unless `GAME_OVER`), `charging/charge`, `mousePos`, field grid not persisted; on resume ball at tee `vel=0` unless `GAME_OVER` then show Game Over screen. Treasure `isCollected` restored for current hole. All numbers clamped `≥0` on load except `totalPoints` may be negative; `maxAttempts` defaults `10` (legacy `>10` kept but never increased); `supply.freeShot` defaults `0` (passive stack), `supply.rotator` defaults `0` (now via starting picks, not auto); `isFreeShotActive` defaults `false`; `treasure` missing → regenerate uncollected; `holeAttempts` defaults `0`; `attemptsLeft = max(0, maxAttempts - holeAttempts)` recomputed, HUD `Attempts Left: X (+Y)` and `Points: N`. Missing fields default `0/false/[]`; corrupt/`version!==1`/`courseId` missing in courses → treat as no save. Wrap in `try/catch`. `PROGRESSION_KEY`/`LOADOUT_KEY` are **removed** per `10-progression.md` — no shop/coins/loadout persistence.
...
- `handleLaunch` (free launches via auto-arm do not decrement `attemptsLeft` but still `saveProgress()` after consuming `freeShot`), `claimReward` (`Free Shot +3` or `Rotator +1` (legacy `Rotate`) or `Field Extender +10%` or `Power Cell +10%`), `rerollReward` (costs 1 counted attempt, never free), `placeModifier`/removal/drag (including `rotator` via `3` and `magnifier` via `4` (legacy `rotate` was `4`)), `collectTreasure`, `advanceHole`/`loadLevel` (including hole-start `rewardPending` and fresh `treasure`), `maybeShowRewardMenu` when creating fresh `rewardOffered`, and on `GAME_OVER`. `toggleFreeShot` via `4` removed (`4` is `rotate`).
...
- Auto-restore: resolve `activeCourse`, `LEVELS=activeCourse.holes`, restore all persisted fields (`maxAttempts`, `holeAttempts`, `supply` with `rotator`/`freeShot` (legacy `rotate`), `isFreeShotActive` via auto-arm only, `treasure.isCollected`, `areaMultiplier`, field `createField` then `setModifiers`, golden glow if `isFreeShotActive`), ball at tee `AIMING` (never `FLYING`/`WIN`) unless saved `GAME_OVER` then `GAME_OVER` overlay (still hide main-menu but bottom shows terrain dimmed). If saved `rewardPending`/`rewardMenuVisible` then show reward menu after restore. Missing fields defaults as above.
- `clearProgress() => localStorage.removeItem(STORAGE_KEY)` and reset run state to `currentHoleIndex=0, holeAttempts=0, totalAttempts=0, maxAttempts=10, supply={1,1,1,1,0}` (`{liquifier:1,deflector:1,rotator:1,magnifier:1,freeShot:0}`), `isFreeShotActive=false`, treasure uncollected (alias `areaUpgradeCount=0`), `powerCellCount=0`, `rewardPending=false`, `rewardOffered=[]`, `modifiers=[]`, clear glow, without touching `COURSES_KEY`. Called on `resetGameAfterWin` (`R` in `WIN`), `startNewGameFromMain` (course play), `endRun` (see §6) and on `Game Over → Return to Main Menu` (see §6). Only full completion updates `bestTotal` (§5), `End Run`/`Game Over` do not. New games start with `1` `rotate` and `0` `freeShot` (HUD `Attempts Left: 10`) and fresh uncollected treasure.
...
- **Pause Menu** (`#pause-overlay`, over playing field, with backdrop `background:rgba(0,0,0,0.55)`): logically separate, triggered while in a level (`activeCourse!==null`, regardless of `AIMING`/`CHARGING`/`FLYING`) by **`Escape` or `P` (`KeyP`)** — both work identically even in `FLYING` (freeze `pos`/`vel`). Pauses `updateBall` but `updateWindUniforms` still runs and field stays rendered dimmed behind backdrop. **Pause shows `Continue`, `Next Attempt`, `Help`, `End Run` in that vertical order** — never the levels list. `Help` in pause is `#pause-help-button` **not circular top-left** (`document.querySelector('#pause-overlay .help-corner-button')===null`, not `?` corner) but regular button `Help` (`textContent==="Help"`) **above `End Run`** (`Help` immediately above `End Run` inside `.pause-content` order `Continue → Next Attempt → Help → End Run`). Pressing `Escape`/`P` again or clicking `Continue` hides `pauseOverlay` and resumes at exact paused state (no `loadProgress` re-parse). **`Next Attempt` button (`#pause-reset-attempt-button`) is placed between `Continue` and `Help` (order: `Continue` → `Next Attempt` → `Help` → `End Run` vertically in `.pause-content`) and when clicked has the same effect as hitting the `R` hotkey (see `05-input-and-states.md` §5: `AIMING`/`FLYING` → `resetBall()` except on last attempt flight where `R` disabled and must play out; `WIN`/`GAME_OVER` → `clearProgress()` → entry menu; `rewardMenuVisible` → `R` is re-roll, not reset — pause never shows `Next Attempt` while `rewardMenuVisible`/`WIN`/`GAME_OVER` as pause cannot be opened in those states; while pause is open `R` handling is blocked, the button replicates the `R` effect by first closing the pause overlay then performing the `R` logic, so the visible result is ball at tee `AIMING` and `pauseOverlay` hidden).** `End Run` shall have red background `background:#e74c3c` (`getComputedStyle` `backgroundColor` `rgb(231,76,60)`) and **confirmation dialog** (`#end-run-confirm-overlay` `position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.55); z-index:16` with card `.end-run-confirm-card` and text asserting end run `Are you sure` + `Confirm`/`Cancel` buttons `#end-run-confirm-ok`/`#end-run-confirm-cancel`; clicking `End Run` shows overlay, `Confirm` executes `endRun()`, `Cancel`/`Escape`/backdrop hides it without ending run).
...
<div id="pause-overlay" class="hidden">
  <div class="pause-content">
    ...
    <button id="pause-next-attempt-button" class="main-menu-button">Next Attempt</button>
    <button id="pause-help-button" class="main-menu-button">Help</button>
    <button id="pause-end-run-button" class="main-menu-button" style="background:#e74c3c">End Run</button>
  </div>
</div>
<div id="end-run-confirm-overlay" class="hidden">
  <div class="end-run-confirm-card">
    <h3>End Run?</h3>
    <p>Are you sure you want to end the run?</p>
    <button id="end-run-confirm-ok">Confirm</button>
    <button id="end-run-confirm-cancel">Cancel</button>
  </div>
</div>
...
- CSS: `#main-menu-overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;width:100%;height:100%;background:transparent;border-radius:8px;z-index:12}` `.with-backdrop{background:rgba(0,0,0,0.55)}` `.hidden{display:none}` etc. End Run red `background:#e74c3c` (`#pause-end-run-button` `rgb(231,76,60)` `2px solid #c0392b`), Help in pause is regular button not `.help-corner-button` (`width:180px` same as others, `background:rgba(255,255,255,0.12)` except End Run is red), not top-left circular.
...
- **Stages list** `#staged-course-list` (inside `#main-menu-root`) bounded to canvas (`max-height: min(65vh,520px)`, `overflow-y:auto`, `display:flex; gap:8px`, `flex-direction:column`, `max-width:480px`, `.course-play-button` `min-width:260px` `overflow:visible`, `.course-name`/`.course-meta` `white-space:nowrap` `overflow:visible`) and **shall not scroll if there is more vertical space in the canvas to occupy** (`scrollHeight <= clientHeight` when content fits; `overflow-y:auto` only scrolls when content exceeds available canvas space). For each `holeCount` in `STAGES=[3,6,9,18]` in order:
...
- **Content:** sacred-links lore verbatim followed by control scheme. Lore normative:
  ...
  By tossing down micro-sized zone emitters anywhere on the course, you can instantly magnify, completely liquify, violently deflect or rotate the wind direction inside a tiny bubble. The Purists scoff at these pocket-sized weather crimes, but as long as you disguise the devices as mundane turf repair tees, the Grand Marshal just assumes you're calling upon the mystical gods of aerodynamics.
  ```
  Rules keywords: `wind`+`hole`/`course`+`attempt` (`wind` in lore, `hole`/`course` in “every hole…anywhere on the course”, `10 attempts`); controls must contain `arrow`/`aim` + `space`/`charge`/`shoot` + `click`/`place`; plus modifiers-before-shoot, `Rotator` (`3` `↻`) and `Magnifier` (`4` `»`) and `Free Shot` passive `Attempts Left: X (+Y)` gold glow (no hotkey) mentioned, per-course `bestTotal` saved via lore context. Controls list rows `Arrow keys — Aim`, `Space — Hold to charge, release to shoot`, `Click — Place modifier`, `Right-click — Remove`, `1 / 2 / 3 / 4 — Select Liquifier / Deflector / Rotator / Magnifier` (legacy `Amplify`→`Magnifier`, `Nullify`→`Liquifier`, `Flip`→`Deflector`, `Rotate`→`Rotator`), `Attempts Left (+Y)` — free shots shown after attempts, `H`, `R`, `Escape/P` pause (core three `Arrow/Space/Click` mandatory) must follow lore in same `.help-card`.
- **Items:** after controls, the same `.help-card` shall contain an **Items section** describing each of the 7 item types with **icon, name and short description of effect**. The section shall be visible without extra interaction (inside scrollable card, not behind a tab) and shall list all 7 types in a vertical list `#help-items.help-items` with one row per type `div.help-item[data-type="magnifier|liquifier|deflector|rotator|fieldExtender|powerCell|freeShot"]` containing an icon `img.help-item-icon` (`src="./img/<type>-icon.png"` for spatial/passive where icon exists, `36×36`/`32×32` `object-fit:contain`, or `★` `36px #FFD700` for `freeShot` if image not used), a name `span.help-item-name` with text exactly `Magnifier`/`Liquifier`/`Deflector`/`Rotator`/`Field Extender`/`Power Cell`/`Free Shot` (case-sensitive), and a short description `span.help-item-desc` (or `p`) explaining the effect:
  - `Magnifier` — Magnifies wind 5× inside bubble (stacking 25×, with Power Cell scaled).
  - `Liquifier` — Nullifies wind to 0 inside bubble and preserves entry velocity; dominates other modifiers, not affected by Power Cell.
  - `Deflector` — Reverses wind direction 180° and magnifies 5× inside bubble (deduped with Rotator to one 5×, further amplified by Magnifier/Power Cell).
  - `Rotator` — Rotates wind 90° CCW and magnifies 5× inside bubble (deduped with Deflector).
  - `Field Extender` — Passive: +15% radius for all spatial modifiers, retroactively grows existing bubbles.
  - `Power Cell` — Passive: +15% strength for Magnifier/Deflector/Rotator (base 5 → 5.75 etc), retroactive.
  - `Free Shot` — Passive: free attempt not counted, shown as `Attempts Left: X (+Y)`, auto-armed on last attempt with golden glow, no placement.
  Each row must show icon + name + description together, with icon left and text right, inside `.help-card` after controls and before Back button, scrollable within card. Legacy icon fallbacks `»/∅/⇄/↻/◯/⚡/★` acceptable for icon if image missing, but `img` with `./img/...` path is normative for spatial/passive (Magnifier/Liquifier/Deflector/Rotator/Field Extender/Power Cell). Description must contain effect keywords (`5×` or `magnif`, `0` or `nullif`/`liquif`, `reverse`/`deflect`/`180`, `rotate`/`90`, `15%`+`radius`/`area`, `15%`+`strength`/`power`, `free`+`attempt` for respective types). The items section title is `h4` text `Items` and follows controls list.
...
- [ ] `Escape`/`P` even in `FLYING` freezes ball, shows pause with backdrop and `Continue` + `Next Attempt` + `End Run` (in that vertical order) plus `Help` round `?` top-left — never `New Game` and never the levels list; `Continue`/`Escape`/`P` unfreezes and ball continues; `Next Attempt` (`#pause-next-attempt-button` / legacy `#pause-reset-attempt-button` between Continue and End Run) has same effect as hitting `R` (resetBall except on last attempt flight where disabled, otherwise same branching including `WIN`/`GAME_OVER` handling via closing pause then performing `R` logic); `End Run` clears `STORAGE_KEY`, restores entry menu over splash. `document.getElementById('pause-next-attempt-button')` (or legacy `pause-reset-attempt-button`) exists and is between `resume-button` and `pause-end-run-button` in DOM order.
