...
For each hole: pick `tee`/`hole` per §2, place 1-2 control points between them with **tier-dependent bigger bends**:
...
- **Medium `L/V/U` with bigger bends**: `L` one offset `>45px` (vs legacy `>30`), single 70-110° bend; `V` acute `30-70°` offset `>45`; `U` two same-side offsets `>45` (70-110) total turn `150-220°`. Medium offsets are `1.4-1.6×` larger than legacy.
- **Hard `S/Z` with even bigger bends + tighter fairway**: `S` two opposite-side offsets `>55px` each (≤80) inflection; `Z` like `S` with sharper `~90°` each bend forming Z. Hard bends `≥20px` larger than medium on average.
...
- **Water blue clusters** (rendered per `03-rendering.md`, fatal per `04-physics-and-collision.md`): small clusters via Cellular Automata (`8×8` grid, `fill 0.42`, 4 smoothing iters) or thresholded Perlin (`>0.6`), partly **on fairway per difficulty** (see §6) with centre `d ≤ W_fairway-10` strictly on fairway (see §6), area `800-3000px²` as rect `w×h` or `r 18-32`, stored `waterHazards`. Never completely block fairway (see Step 5).
- **Trees** (`type:'circle' r 18-36`, trunk `#6B3A2A` canopy `#1E7A34`): partly **on fairway** per tier (see §6) with `treesOnFairway` strictly on fairway (`terrainZoneAt==='fairway'` and `d ≤ W_fairway-4` not in Green/Tee mask). All fairway trees respect `≥40` clearance from tee/green masks and `≥ r1+r2+6` between trees (Poisson Bridson `minDist 45±15 k=30` or `OB 38`/`Rough 62` / `p=0.55` density split). Additional trees for aesthetics may be placed in Rough/OB (`d > W_fairway`, `OB 1.8× Rough` density) respecting same clearance; total may be `8-22` but **normative counts are `treesOnFairway` per tier**.
...
- [ ] Every hard flipped has `sources=2,sinks=2` all `20-60` outside; medium never places sink on green-closest edge; sources/sinks never exactly on edge; ≥1 doublet in fairway tree when `trees≥1`.
