...
- Counters and `supply`/`maxAttempts`/`isFreeShotActive`/`areaUpgradeCount`/`reward…`/`treasure`/`modifiers`/`aimAngle` are part of `STORAGE_KEY` payload and re-saved after every launch/claim/reroll/placement/treasure-hit/advance (see `09-persistence-and-campaign.md`). Legacy `bouncyBallCount` removed (kept for compat). `freeShot` starts `0`, `rotate` starts `1`; legacy saves default `0`/`1`.
...
- [ ] Launches: normal increments `holeAttempts`/`totalAttempts`; free launch via auto-arm does not and persists while supply remains; last-attempt failure immediate `GAME_OVER` without `resetBall()`; fallback `handleLaunch` while `attemptsLeft<=0` also shows Game Over; treasure hit never increments.
