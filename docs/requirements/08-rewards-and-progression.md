...
- **Inside-canvas overlay** `drawRewardMenu(ctx,W,H,offered,hovered,rerolled,rerollHovered)` when `rewardMenuVisible`: dim `rgba(0,0,0,0.55)`, title `Choose an Upgrade` 22px `700` white `stroke 5px` centered, three `90×110` buttons centered (`340×220` gap 12):
  - `amplify` orange `#e67e22`, `nullify` blue `#3498db`, `flip` purple `#9b59b6`, `rotate` red `#e74c3c` `↻` hint `+1 to supply`, `freeShot` gold `★` `#f1c40f` hint `Supply +3`, `fieldExtender` amber `◯` `#f39c12` hint `+10% area` (was `+20%`), `powerCell` violet `⚡` `#8e44ad` hint `+10% strength`. All labels white with stroke `4px`; no `Total Attempts: N` subtitle. Legacy `areaUp` label `Area +20%` still renders as `Field Extender +10%` if legacy offer contains `areaUp`.
