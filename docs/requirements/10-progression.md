...
- **Overlay** `#coin-summary-overlay` is a **full screen overlay with transparent black backdrop** (`position:absolute; inset:0; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,0.55); border-radius:8px; z-index:14` above main menu but below help), shown **on top of the main menu splash (no main menu buttons visible, only splash background)** with a black transparent backdrop **similar as the loadout screen** (`#loadout-overlay`). While `coinSummaryVisible===true`, `mainMenuVisible===true` remains true for splash, but `#main-menu-root`/`#course-menu`/`#help-button`/`#menu-title`/`#campaign-seed-wrapper` are hidden (`.hidden`) so only `parallax-scene` splash is visible behind the `0.55` dim. Card structure:
```html
...
- **Breakdown rows** below the moneybag: first row exactly `10💰 - Hole Completed (x3)` format — must contain `10💰`, `-`, `Hole Completed` and `(x3)` / `(xN)` count suffix. Example `10💰 - Hole Completed (x3)` for 3 holes, `10💰 - Hole Completed (x1)` for 1 hole. Normative: visible row inside `#coin-summary-details` must contain `10💰` and `Hole Completed` (case-insensitive) and `-` and `(x` with the hole count (e.g. `(x3)`, moneybag `💰` **before** `-` i.e. `10💰 -`). **Second row when course was cleared (bonus awarded, `+50`): exactly `50💰 - Course Completed` (must contain `50💰`, `-`, `Course Completed`, moneybag `💰` **before** `-` i.e. `50💰 - Course Completed`) as second `.coin-detail-row` inside `#coin-summary-details` when `isCourseComplete` (`coins === holes*10+50`). Accept `Couse Completed` typo as alias but canonical is `Course Completed`. When course not cleared, only the first row is shown.** Do **not** show legacy `Hole cleared - 10💰 (xN)` / `Course cleared, 50💰` / `0 hole clears × 10`, `You earned 30 coins: 3 holes × 10 coins per hole` or `3 holes cleared — 10 per hole` in the overlay (`#coin-summary-text` and `#coin-summary-breakdown` shall be hidden `display:none` or not visible, `getComputedStyle === none` or `hidden` attribute, and not containing that phrasing, and do not show `×` style). Only the big `+X💰` and the new breakdown(s) remain.
...
- Do **not** show legacy texts `You earned` or `holes cleared — 10 per hole` or `0 hole clears × 10` / `×` style nor old `Hole cleared - 10💰` / `Course cleared, 50💰` in the visible overlay (hide those elements). Only show big `+X💰` and `10💰 - Hole Completed (xN)` / `50💰 - Course Completed` plus unlock phrase.
- On confirm (`#coin-summary-ok` click or `Escape` or `Enter`), hide overlay (`classList.add('hidden')`) and remain on main menu. Coins already persisted. While `coinSummaryVisible`, main menu splash remains visible behind (`parallax-scene` not hidden) but buttons hidden.
- **Styling**: card **transparent** (`background:transparent`/`none`), title white `stroke 5px`, big amount `+X` before `💰` prominent, rows smaller `500-600 12-13px` muted white, gap `16-20px`, overlay `inset:0` full screen, centered with `0.55` backdrop like loadout.
...
- [ ] Coin earning: `COINS_PER_HOLE=10` + `COURSE_COMPLETE_BONUS=50` on full course clear. Clearing 2 holes → `runCoinsEarned 20` (no course bonus, only one row `10💰 - Hole Completed (x2)`); completing 3-hole course → `80` (`3×10+50`) and summary appears over main menu splash **with `0.55` backdrop (no main menu buttons, only splash, like loadout) and transparent card (no `rgba(0,0,0,0.75)` black behind, `getComputedStyle(.coin-summary-card).backgroundColor` is `transparent`/`rgba(0,0,0,0)`)** with title `Run Complete`, **big `+80💰` with number before icon and plus** (`#coin-summary-amount` text `+80` before `.coin-big-icon` `💰` in `.coin-summary-big`, `font-size 44px` smaller), **first row `10💰 - Hole Completed (x3)`** (`#coin-summary-details .coin-detail-row:first-child` contains `10💰`, `-`, `Hole Completed`, `(x` e.g. `10💰 - Hole Completed (x3)` — must contain `10💰` before `-` and `Hole Completed`) and **when course cleared second row `50💰 - Course Completed`** (`#coin-summary-details .coin-detail-row:nth-child(2)` contains `50💰`, `-`, `Course Completed`/`Couse Completed`, e.g. `50💰 - Course Completed` — only when `isCourseComplete`/bonus awarded, otherwise only one row) **and conditionally bigger unlock `Loadout slot unlocked`** (`#coin-summary-unlock` text exactly `Loadout slot unlocked`, `font:700 15px`, **only visible when a new loadout slot was actually unlocked in that run** `getUnlockedLoadoutSlots() > loadoutUnlockedAtRunStart` — e.g. after first 3-hole clear shows, after End Run with 0 holes no overlay at all), **do not show** old `Hole cleared - 10💰` / `Course cleared, 50💰` nor `0 hole clears × 10`, `You earned 30 coins: 3 holes × 10 coins per hole` nor `3 holes cleared — 10 per hole` (`#coin-summary-text`/`#coin-summary-breakdown` hidden `display:none`, and no `×` style). `runCoinsEarned` resets. **End Run 0 holes (coins 0) does not show overlay at all** (`#coin-summary-overlay` stays `hidden`, `isCoinSummaryVisible()===false`). **Overlay shown on top of main menu splash with no buttons, `background:rgba(0,0,0,0.55)` like loadout, card transparent.**
