...
2. **Bottom Canvas Rendering — Background Images Only, No Backdrop Dimming** in `src/render.js` / `src/main.js`:
   ...
   - **When on the main menu** (`mainMenuVisible===true`, root or course submenu or help):
     - Bottom canvas SHALL show `gfg-splash.png` **aspect-covered, centered** (`Math.max(W/imgW,H/imgH)` scale, centered `drawImage`), not tiled. No grass SHALL be visible while menu is shown.
     - **No dimming backdrop SHALL be drawn over the splash**: `bgCtx` SHALL NOT fill a semi-transparent dark rect over the splash, and `#main-menu-overlay` SHALL have `background: transparent` (not `rgba(0,0,0,0.35)`). The splash SHALL be fully visible; opaque buttons (REQ-029) provide contrast. A previous spec's `background:rgba(0,0,0,0.35)` on the overlay is **removed / forbidden**.
...
6. **Main Menu HTML — Centered Over Canvas, Not Overflowing, No Backdrop** (normative cross-ref to REQ-029):
   - `#main-menu-overlay` SHALL be inside `#game-container` with `position:absolute; inset:0; display:flex; align-items:center; justify-content:center; width:100%; height:100%; background:transparent;` so its box equals the 16:9 canvas area — never larger. Content `.main-menu-content` SHALL have `max-width:90%; max-height:90%;` to prevent overflow. No dimming `rgba(0,0,0,0.35)` SHALL be present on the overlay — tests SHALL assert `getComputedStyle(overlay).backgroundColor === "rgba(0, 0, 0, 0)"` or `transparent`.
