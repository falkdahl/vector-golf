...
2. **Placement Interaction** in `src/input.js` and `src/main.js`:
   ...
   - Right-click or `Delete`/`Backspace` or clicking an existing modifier SHALL remove it (optional but recommended for misclick correction). Alternatively, `R` resets all modifiers for current hole along with ball.
   - Placement SHALL respect limits: max `3` modifiers per hole (configurable `MAX_MODIFIERS_PER_HOLE=3` in `src/levels.js` or `src/main.js`). If limit reached, oldest modifier is replaced or placement is ignored with feedback (shake hotbar).
   - Modifiers SHALL be stored as array `modifiers = [{id, type:'amplify'|'nullify'|'flip', x, y, radius:90}]` in `src/main.js` or `src/vectorField.js`.
...
4. **Visualization Integration** in `src/render.js`:
   - Placed modifiers SHALL be drawn as circles on the canvas (below ball but above arrows/particles): `amplify` solid orange ring `rgba(230,126,34,0.25)` with `>>` icon, `nullify` blue dashed `rgba(52,152,219,0.25)` with `∅`, `flip` purple double arrow `rgba(155,89,182,0.25)` with `↻`.
   - Preview circle while hovering (no placement) SHALL be dashed 50% opacity.
   - Arrow grid and particles SHALL reflect modified field in real-time (since they use `getWindAt`, they automatically show altered vectors inside modifier circles).

5. **Determinism** per REQ-003: modifiers SHALL be deterministic for the current hole; reloading the hole resets them.

## Acceptance Criteria

- [ ] Hotbar with 3 slots visible below canvas (or bottom of canvas) showing Amplify x5, Nullify, Flip with distinct colors/icons and selection highlight.
...
- [ ] Left-click places modifier at cursor; it remains drawn as solid circle after click.
...
- [ ] Hotbar hidden during `FLYING`; cannot place while ball drifting.
- [ ] Right-click on modifier removes it; `R` (ball reset) does not clear modifiers, but advancing hole or game reset does.
- [ ] Max 3 modifiers per hole enforced; 4th placement either ignored or replaces oldest with visual feedback.
- [ ] Particles and arrows inside modifier circles visibly change (e.g., arrows longer/brighter inside amplify, zero inside nullify, reversed inside flip).
