...
- **Sink — every level has at least one sink** (`sinks ≥1`, actually `sinks=1` for all tiers) placed **slightly above the canvas upper edge** (`y=-OUTSIDE_SINK`) **or slightly below the lower edge** (`y=H+OUTSIDE_SINK`) **on the middle third of the canvas** (`x ∈ [W/3, 2*W/3]`, `OUTSIDE_SINK 60-100`), `rand()<0.5` chooses top vs bottom. Guarantees `sink.x ∈ [W/3, 2*W/3]` (`x ∈ [426,853]` at `W=1280`) and `sink.y <0` or `sink.y >H` with **increased distance 60-100** from edge (was 20-60, was right third `[853,1280]`). Applies to **all tiers** (easy/medium/hard).
- **Easy — no head wind (tail wind left→right)**:
  ...
  - Sink as above (middle third top/bottom, **60-100** outside). Guarantees `source.x < W/3` and `sink.x ∈ [W/3,2*W/3]`; tail wind with sink in middle, no head wind. `1,1,1,0` total 3.
- **Medium — extra source, single sink on middle-third top/bottom (increased distance)**:
  ...
  - Sink **single** as above (middle third top/bottom, **60-100** outside) — always `sinks=1`. `2,1,2,1` total 6.
- **Hard — single sink on middle-third top/bottom (increased distance)**:
  - Source slightly **outside edge closest to `tee`** (same as medium without extra, `OUTSIDE 20-60`).
  - Sink **single** as above (middle third top/bottom, **60-100** outside) — **no extra sink/source** (removed, always `1,1` not `2,2`; flipped logic removed). `1,1,3,1` total 6.
...
- **Particles only**: `PARTICLE_COUNT=60-80` (default `70`), spawned uniformly in `[0,W]×[0,H]`, advected by `getWindAt(pos)` of the **same field components including live modifiers** (so trails visibly follow modifiers), `particleSpeed ~40-60` or `length(vWind)*k`. Distribution stays even; OOB or life expiration triggers uniform-random respawn (no wrap, no edge clumping).
  ...
  - **Modifier-aware**: inside `amplify` trails ~5× faster/longer, inside `nullify` stall, inside `flip` reverse and 5× faster (flip now `*-5`, same amplification as amplify), inside `rotate` 90° CCW and 5× faster (rotate includes one amplify; stacked `rotate`/`flip` deduped to one 5×, further amplified by `amplify`).
...
- [ ] `createField` signature requires `seed`+four counts (no unary); coerced outside/inside placement verified for 100 random seeds (all sources `20-60` outside, all sinks `60-100` outside on middle-third top/bottom (`x∈[W/3,2*W/3]`, `y=-60..-100` or `H+60..H+100`, increased distance, was right-third), all vortexes/doublets `≥20` inside, ≥1 vortex|doublet inside except hole-1).
