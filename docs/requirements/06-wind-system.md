...
- **Free Shot golden glow + edge glow** (see `07-modifiers.md` §5.2): while `isFreeShotActive===true` (armed) **or while ball is in motion after a free launch** (`freeShotFlightActive`), `src/windThree.js` renders an additional **golden glowing effect centered on `ball.pos`** in the Three.js layer (`#wind-canvas`) **and a subtle gold edge glow around the canvas**. Implementation: `THREE.Sprite` or `THREE.Mesh` (`CircleGeometry`/`SphereGeometry`) with `MeshBasicMaterial`/`SpriteMaterial` `color 0xf1c40f` (`#f1c40f`/`0xFFD700`), `transparent:true`, `opacity 0.55-0.85`, `blending:THREE.AdditiveBlending`, `depthWrite:false`, `radius 14-22px` logical (DPR-scaled), soft radial falloff texture, pulsation `scale 1.0 + 0.15*sin(time*3)` and `opacity` flicker, plus DOM `div#free-shot-edge-glow` `position:absolute; inset:0; border-radius:8px; border:2px solid rgba(241,196,15,0.38); box-shadow: inset 0 0 36px rgba(241,196,15,0.22), inset 0 0 70px rgba(241,196,15,0.12), 0 0 18px rgba(241,196,15,0.28); opacity 1 when active`. Export `setFreeShotActive(active:boolean)` / `updateFreeShotGlow(ballPos, dt)` called from `src/main.js` when toggling `isFreeShotActive`/`freeShotFlightActive` and each `updateWind`/`renderWind` frame (including `FLYING`). Glow is hidden when neither armed nor in free flight (cleared after free launch lands, after hole advance, on `GAME_OVER`, or when `rewardMenuVisible`). `renderWind` must render even when `H` hides particles if glow active. Does not affect particle simulation; purely visual feedback that next attempt is/was free.
