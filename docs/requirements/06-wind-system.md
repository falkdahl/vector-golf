...
- **Grid:** inside `#hotbar` child `div#hotbar-grid` `display:grid; grid-template-columns:repeat(6,1fr); gap:6px;` containing six squares: four active `div.hotbar-slot[data-type="amplify|nullify|flip|rotate"]` + two passive `div.hotbar-slot[data-type="fieldExtender|powerCell"].passive` each **`40×40px`** (net `+12.5%` vs original `35px` after `+50%` then `-25%`; `±2px` tolerance, `aspect-ratio:1/1`), `border-radius:6px; border:2px solid <distinct>; position:relative; overflow:visible; display:flex; align-items:center; justify-content:center; cursor:pointer; user-select:none` with **distinct border per type:** `amplify #7a3a0a` on orange, `nullify #1a4a6b` on blue, `flip #4a235a` on purple, `rotate #6e1a12` on red, `fieldExtender #b7791f` on amber `rgba(243,156,18,0.82)`, `powerCell #7d3c98` on violet `rgba(142,68,173,0.82)` (passive, `cursor:default`, no selection). Max-width of `#hotbar` increased to `max-width:420px` to fit 6 slots (was `300px` for 4).
  - Each square **slightly transparent background in the modifier's respective color:** `amplify` orange `rgba(230,126,34,0.82)`, `nullify` blue `rgba(52,152,219,0.82)`, `flip` purple `rgba(155,89,182,0.82)`, `rotate` red `rgba(231,76,60,0.82)`, `fieldExtender` amber `rgba(243,156,18,0.82)`, `powerCell` violet `rgba(142,68,173,0.82)`. Icon centered `img.hotbar-icon-img` `28×28px` (increased from `22×22`, squares stay `40×40`) `object-fit:contain` `pointer-events:none` `filter:drop-shadow(0 1px 2px rgba(0,0,0,0.35))` with `src` `❰./img/amplify-icon.png❱` / `nullify-icon.png` / `flip-icon.png` / `rotate-icon.png` / `field-extender-icon.png` / `power-cell-icon.png` (relative `./img/...`). Text icon fallback `span.hotbar-icon` removed. Canvas field circles (`drawModifiers`/`drawModifierPreview` on `fgCtx`) keep text icons `»/∅/⇄/↻` (see §7) — hotbar/reward use images, field keeps text. `img` keeps color identity background visible.
...
- State `fieldExtenderCount >=0` alias `areaUpgradeCount` (`0` on new game) defined in `08-rewards-and-progression.md` but applied here. Renamed from `Area Up` to **Field Extender**, effect reduced to **+10%** (was +20%). Multiplier `areaMultiplier = 1 + 0.1*fieldExtenderCount` (additive, not `1.1^n`; e.g. `54*1.2=64.8` for 2). Legacy `areaUpgradeCount`/`areaUp` aliases kept for backward compat.
...
- `freeShot` unaffected.
- **Power Cell — modifier strength:** State `powerCellCount >=0` (`0` on new game) defined in `08-rewards-and-progression.md`. `BASE_STRENGTH=5`. Strength multiplier `powerMultiplier = 1 + 0.1*powerCellCount` (additive, not `1.1^n`). Effective base strength `effectiveBase = BASE_STRENGTH * powerMultiplier` (e.g. `5*1.2=6` for 2). Applied to **amplify, flip and rotate** modifiers per §7.1. `nullify` and base wind unaffected. Retroactive: existing modifiers' strength grows on `Power Cell` pickup.

## 7. Per-Type Effects

### 7.1 Spatial Wind Effects (applied in `getWindAt` after base field)

- **Amplify (`5×` per circle, scaled by Power Cell):** `if amplify && inside → wind*= effectiveBase` where `effectiveBase = BASE_STRENGTH * (1+0.1*powerCellCount)` (`5` → `5.5` with 1 Power Cell, `6` with 2, etc.). Stacking `5×`, `25×`, `125×` scaled by `powerMultiplier` (see §6). With 1 Power Cell stacking `5.5×`, `30.25×`, `166×` if per-modifier scaled, or `5×*1.1=5.5×`, `25×*1.1=27.5×` if global — normative is global `powerMultiplier` multiplying final `5**n` factor: `totalFactor = (5**(amplifyCount+(hasRotFlip?1:0))) * (1+0.1*powerCellCount)` when any amplify/rot/flip active (if no amplify/rotFlip, `powerMultiplier` not applied).
...
- [ ] `getWindAt` inside `amplify` `5×*powerMultiplier`, `nullify` `{0,0}` (physics preserves velocity, unaffected by Power Cell), `flip` `*-5*powerMultiplier`, `rotate` `90° CCW 5×*powerMultiplier`; stacking deduped `flip+flip→5×*pm`, `rotate→5× 90°*pm`, `amplify+flip→-25×*pm` etc.; effective radius `BASE_MODIFIER_RADIUS*(1+0.1*fieldExtenderCount)` used for all spatial; `freeShot` does not affect `getWindAt`; with `powerCellCount=1` amplify `5× → 5.5×`, `powerCellCount=2 → 6×` (global `powerMultiplier=1+0.1*n` when any amplify/rot/flip active).
