...
- **Hotbar grid — Transparent background (same as reward menu):** inside `#bottom-bar` sibling `div#hotbar` (`display:flex; align-items:center; gap:6px; z-index:5`) with **transparent** `background:transparent` `border:none` `box-shadow:none` `backdrop-filter:none` `border-radius:10px; padding:6px; max-width:300px; transition: padding 0.18s ease, background 0.18s ease, transform 0.18s ease, opacity 0.18s ease, border 0.18s ease` (plus expand/collapse transitions below). The tight gap and middle alignment inside the bottom-centered wrapper make bag+hotbar read as a single grouped control. No black container background (same as reward menu slots). Bag+hotbar are **always visible during gameplay including `FLYING` (shot in flight)** — they do not disappear when ball is launched; they remain visible through the entire hole until `WIN`/`GAME_OVER`/`pause`/`main-menu`. During reward selection both bag and hotbar remain visible.
  - **Grid:** inside `#hotbar` child `div#hotbar-grid` `display:grid; grid-template-columns:repeat(6,1fr); gap:6px;` containing six squares: four active `div.hotbar-slot[data-type="amplify|nullify|flip|rotate"]` + two passive `div.hotbar-slot[data-type="fieldExtender|powerCell"].passive` each **`54×54px`** (`±2px` tolerance, `aspect-ratio:1/1`), `border-radius:6px; border:2px solid <distinct>; position:relative; overflow:visible; display:flex; align-items:center; justify-content:center; cursor:pointer; user-select:none` with **distinct border per type:** `amplify #7a3a0a` on orange, `nullify #1a4a6b` on blue, `flip #4a235a` on purple, `rotate #6e1a12` on red, `fieldExtender #7a7a7a` on gray `rgba(128,128,128,0.82)`, `powerCell #7a7a7a` on gray `rgba(128,128,128,0.82)` (passive, `cursor:default`, no selection, gray background for both passives). Max-width of `#hotbar` increased to `max-width:420px` to fit 6 slots (was `300px` for 4).
   - Each square **transparent background same as reward menu (0.28):** `amplify` orange `rgba(230,126,34,0.28)`, `nullify` blue `rgba(52,152,219,0.28)`, `flip` purple `rgba(155,89,182,0.28)`, `rotate` red `rgba(231,76,60,0.28)`, `fieldExtender` gray `rgba(128,128,128,0.28)`, `powerCell` gray `rgba(128,128,128,0.28)`. Icon centered `img.hotbar-icon-img` `36×36px` (squares `54×54`) `object-fit:contain` `pointer-events:none` `filter:drop-shadow(0 1px 2px rgba(0,0,0,0.35))` with `src` `❰./img/amplify-icon.png❱` / `nullify-icon.png` / `flip-icon.png` / `rotate-icon.png` / `field-extender-icon.png` / `power-cell-icon.png` (relative `./img/...`). Text icon fallback `span.hotbar-icon` removed. Canvas field circles (`drawModifiers`/`drawModifierPreview` on `fgCtx`) keep text icons `»/∅/⇄/↻` (see §7) — hotbar/reward use images, field keeps text. Active squares keep color identity background visible; passive squares have gray background.
...
- `getEffectiveModifierRadius() = BASE_MODIFIER_RADIUS * areaMultiplier`. Applies to spatial `amplify`/`nullify`/`flip`/`rotate`. Aliases `getAreaMultiplier`/`getAreaUpgradeCount` kept.
- All hit tests shall use effective radius. Existing modifiers grow retroactively on `Field Extender`.
- `freeShot` unaffected.
- **Power Cell — modifier strength:** State `powerCellCount >=0` (`0` on new game) defined in `08-rewards-and-progression.md`. `BASE_STRENGTH=5`. Strength multiplier `powerMultiplier = 1 + 0.15*powerCellCount` (additive, not `1.15^n`). Effective base strength `effectiveBase = BASE_STRENGTH * powerMultiplier` (e.g. `5*1.3=6.5` for 2). Applied to **amplify, flip and rotate** modifiers per §7.1. `nullify` and base wind unaffected. Retroactive: existing modifiers' strength grows on `Power Cell` pickup.

## 7. Per-Type Effects

### 7.1 Spatial Wind Effects (applied in `getWindAt` after base field)

- **Amplify (`5×` per circle, scaled by Power Cell):** `if amplify && inside → wind*= effectiveBase` where `effectiveBase = BASE_STRENGTH * (1+0.15*powerCellCount)` (`5` → `5.75` with 1 Power Cell, `6.5` with 2, etc.). Stacking `5×`, `25×`, `125×` scaled by `powerMultiplier` (see §6). With 1 Power Cell stacking `5.75×`, `33.06×`, `190×` if per-modifier scaled, or `5×*1.15=5.75×`, `25×*1.15=28.75×` if global — normative is global `powerMultiplier` multiplying final `5**n` factor: `totalFactor = (5**(amplifyCount+(hasRotFlip?1:0))) * (1+0.15*powerCellCount)` when any amplify/rot/flip active (if no amplify/rotFlip, `powerMultiplier` not applied).
- **Nullify (`0`):** `getWindAt` returns `{0,0}` for visualization; physics inside nullify skips wind and friction (preserves entry velocity). Dominates over others. Violates minimum-force rule intentionally. **Not affected by Power Cell.**
- **Flip (`-5×` includes one amplify, scaled by Power Cell):** `wind = (-wind)* effectiveBase`. Stacked flips/rotates deduped: any number of `flip`+`rotate` combined contribute exactly one `effectiveBase` regardless of count (e.g. `flip+flip→5×*powerMultiplier` not `25×`). Further amplified by explicit `amplify`: `amplify(5)+flip(-5)→-25×*powerMultiplier`.
- **Rotate (`90° CCW` + `5×`, scaled by Power Cell):** `wind = rotate90CCW(wind)* effectiveBase` where `rotate90CCW(x,y)=(-y,x)`. Deduped with `flip`: any `rotate`+`flip` together give one `effectiveBase`; angle stacks `rotateCount*90° + flipCount*180°` modulo 360°, magnitude stays `effectiveBase` once. Explicit `amplify` further amplifies: `amplify+rotate→25×*powerMultiplier 90° CCW`, `2×amplify+rotate+flip→125×*powerMultiplier 270° CCW`. Nullify still dominates.
- **Normative formula:** `BASE_STRENGTH=5`, `powerMultiplier=1+0.15*powerCellCount`, `amplifyCount=|{m:inside∧amplify}|`, `flipCount=|{m:inside∧flip}|`, `rotateCount=|{m:inside∧rotate}|`, `hasRotFlip=(flipCount+rotateCount)>0`, `totalFactor=(5**(amplifyCount+(hasRotFlip?1:0))) * (hasRotFlip||amplifyCount>0 ? powerMultiplier : 1)`, `totalQuarterTurns=(rotateCount+2*flipCount)%4`, `wind=(base*totalFactor) rotated Q*90° CCW`. Nullify first. Circular area `dist<effectiveRadius` (`effectiveRadius = BASE_MODIFIER_RADIUS * (1+0.15*fieldExtenderCount)`). With `powerCellCount=0` reduces to legacy `5**n`.
...
- [ ] `getWindAt` inside `amplify` `5×*powerMultiplier`, `nullify` `{0,0}` (physics preserves velocity, unaffected by Power Cell), `flip` `*-5*powerMultiplier`, `rotate` `90° CCW 5×*powerMultiplier`; stacking deduped `flip+flip→5×*pm`, `rotate→5× 90°*pm`, `amplify+flip→-25×*pm` etc.; effective radius `BASE_MODIFIER_RADIUS*(1+0.15*fieldExtenderCount)` used for all spatial; `freeShot` does not affect `getWindAt`; with `powerCellCount=1` amplify `5× → 5.75×`, `powerCellCount=2 → 6.5×` (global `powerMultiplier=1+0.15*n` when any amplify/rot/flip active).
