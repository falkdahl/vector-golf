...
1. **Trigger Condition** in `src/main.js`:
   ...
   - Trigger moments:
     - On **new game** (`totalAttempts === 0`) immediately after `initLevel()` / `loadLevel(0)` and before first aiming input — player starts with empty supply and must choose first upgrade among three random options.
...
3. **Inside-Canvas Rendering** in `src/render.js`:
   ...
   - Visual spec (no white background, high contrast on green `#3a9d23`):
     ...
     - Each displayed button SHALL use the same styling as above; the excluded fourth type SHALL not be drawn.
...
4. **Selection & Mutation** in `src/main.js` + `src/render.js` hit-testing:
   ...
   - On selection:
     ...
     - In both branches: `rewardClaimedFor = totalAttempts` (or add to `Set`) and `rewardMenuVisible = false` and `rewardOffered = []` cleared.
     - Menu SHALL close immediately; mouse cursor returns to `default`; normal `AIMING` input resumes. No additional placement is performed — modifier upgrades still require explicit hotbar selection; free shots are consumed automatically on next launches per REQ-022.
   - Selection SHALL be idempotent: rapid double-click SHALL only grant `+1` (or `+3` for free shots) once per trigger. The two mutation paths are mutually exclusive per selection (never both). Random selection SHALL be distinct — no duplicate types in the same menu.

5. **Lifecycle & Persistence** per REQ-014/REQ-020/REQ-022:
   - `rewardMenuVisible` SHALL be `false`, `rewardClaimedFor` cleared, and `rewardOffered` cleared on **new game** (`resetGameAfterWin()` or `currentHoleIndex=0, totalAttempts=0, holeAttempts=0, freeShots=0` reset, and page reload). This causes the initial `0` reward to appear again as a new random 3-set on fresh start.
   - Death resets (`resetBall()`), `R` during play, and hole advances (`advanceHole()`/`handleNextHole()`/`loadLevel(n>0)`) SHALL NOT reset `supply`/`freeShots` and SHALL NOT clear `rewardClaimedFor`/`rewardOffered` except when advancing coincides with a new qualifying `totalAttempts` value. If advancing to a new hole leaves `totalAttempts %5===0` and that value was already claimed, no second menu SHALL appear for same value.
   - `FLYING` → `AIMING` transition via `resetBall()` after OOB/obstacle SHALL evaluate trigger before next launch.
   - `freeShots` granted via menu SHALL persist per REQ-022 (through death and hole advances) until consumed.
...
- [ ] On fresh page load (`totalAttempts=0`, new game) the game shows a canvas-drawn upgrade menu centered **inside the canvas** (not DOM) before any aiming: dim `rgba(0,0,0,0.55)` over green, **no white card background**, title `Choose an Upgrade` 22px bold white with dark stroke, **exactly three buttons** (not four) with high-contrast white labels; buttons are three distinct types randomly chosen from the pool `Amplify »` (orange), `Nullify ∅` (blue), `Flip ⇄` (purple), `Free Shots +3 ★` (green `#2ecc71`), each with `+1 to supply` or `+3 free shots` hint and key hints `[1]`,`[2]`,`[3]` matching left-to-right order, no `Total Attempts: 0` text.
...
- [ ] Randomness: reloading the page 10 times at `Total=0` yields varying 3-sets over the runs (not always the same three) and never shows duplicate types within a single menu (three distinct). All four pool types appear across runs (statistical; e.g., at least once in 10). Similarly, reaching `Total=5` after 5 counted launches shows a new random 3-set independent of the `0` set.
- [ ] Exactly three buttons are hit-testable; the fourth pool type not offered is not drawn and not clickable. Clicking outside buttons while menu open does not place a modifier.
- [ ] While menu is open at `0`, pressing `Space` does NOT charge/launch, pressing `ArrowLeft`/`ArrowRight` does NOT rotate aim, hotbar `1`/`2`/`3` as placement is blocked — only menu `1`/`2`/`3` selects the offered upgrades by position (e.g., if offered is `[flip, freeShots, amplify]`, pressing `1` grants `Flip`, `2` grants `+3 free shots`, `3` grants `Amplify`).
- [ ] Selecting an offered `Amplify` (click on its button or press its positional key `1`-`3`) closes the menu, `supply.amplify` becomes `1` (was `0`), other supplies and `freeShots` unchanged, hotbar badge updates, and normal aiming resumes. Similarly selecting offered `Nullify`/`Flip` grants `+1` to that type only.
- [ ] When `Free Shots +3` is among the three offered, selecting it (click or positional `1`-`3`) closes the menu, `freeShots` becomes `3` (`getFreeShots()===3`), `supply` all remain `0`; if not offered, it cannot be selected and `freeShots` stays `0` until a future menu where it appears.
- [ ] After claiming at `0` and launching 5 counted shots (`totalAttempts` `1→5` if no free shots consumed), at `totalAttempts=5` and ball reset to tee, the upgrade menu appears again with a new random 3-set before the 6th counted attempt; claiming marks `rewardClaimedFor=5` and no second menu appears for `5` on `R` resets.
- [ ] If `Free Shots +3` was taken and is among offered, the next 3 launches consume free shots (`freeShots 3→0`) and do NOT increment `totalAttempts`; the menu for `5` is delayed until 5 counted attempts have occurred. When menu appears at `5` with a new random 3-set, choosing `Free Shots +3` (if offered there) adds `+3` additively (e.g., `0→3` or `1→4`).
- [ ] Pressing `R` in `WIN`/`GAME_COMPLETE` (`resetGameAfterWin`) resets `totalAttempts=0`, `supply={0,0,0}`, `freeShots=0`, clears `rewardClaimedFor`/`rewardOffered`, and shows a new random 3-set again at `0`.
...
- [ ] No 3rd-party libraries; vanilla JS `Math.random()` shuffle, `totalAttempts %5`, `supply[t]++` or `freeShots+=3`, `rewardMenuVisible` boolean.
...
- Implementation sketch in `src/main.js`:
  ```js
  const POOL = ['amplify','nullify','flip','freeShots']; // 4 possible
  ...
  function maybeShowRewardMenu() {
    ...
    rewardOffered = shuffle([...POOL]).slice(0,3); // 3 random
...
- `src/main.js:1` (POOL, rewardOffered, rewardMenuVisible, rewardClaimedFor, shuffle+maybeShow, claimReward branching, keyboard 1/2/3 → offered order, resetGameAfterWin clear)
- `src/render.js:1` (drawRewardMenu - no white background, title Choose an Upgrade, getRewardButtonsLayout(offered) returns 3 buttons, high-contrast white text)
