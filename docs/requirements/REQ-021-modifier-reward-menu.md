...
1. **Trigger Condition via Secret Counter** in `src/main.js`:
   ...
   - The secret counter SHALL **persist** through death resets (`resetBall()` on obstacle/OOB), `R` during play, and hole advances (`advanceHole()`/`handleNextHole()`/`loadLevel(n>0)`) — those SHALL NOT reset it. Only reaching `5` (reset to `0` + show menu) and **new game** (reset to `0` with no pending menu) SHALL reset/clear it. No `localStorage` required beyond REQ-027 persistence.
   ...
   - There SHALL be **no special trigger before the first counted shot**. New game SHALL start with `rewardMenuVisible=false`, `rewardPending=false`, and no pending reward; `maybeShowRewardMenu()` SHALL NOT show a menu when `totalAttempts===0` and `secretRewardCounter===0`.
...
- [ ] On fresh page load (new game, `secretRewardCounter=0`, `totalAttempts=0`, `supply={1,1,1}`) the game does **NOT** show a reward menu before the first attempt. Canvas shows fairway with HUD `Total:0` and no dim overlay; `rewardMenuVisible===false`, `rewardPending===false`, hover/mouse is in aiming mode and hotbar shows `1` for each modifier. Supply is `{1,1,1}` and one of each modifier can be placed immediately.
...
- `src/main.js:1` (POOL 6 with bouncyBall, secretRewardCounter hidden, rewardPending, **rewardRerolled**, `rerollReward()` per REQ-025, maybeShowRewardMenu for secret counter only, handleLaunch increments secret counter only on non-free shots and resets at 5, claimReward, `R` re-bound to re-roll when menu visible, resetGameAfterWin/startNewGameFromMain/endRun/clearProgress clears secret counter + reroll flag, etc.)
