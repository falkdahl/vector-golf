# REQ-013: Responsive Canvas & HiDPI Handling — 16:9 Maximized Stacked Canvases, Only Canvases, No Outside Elements

- **ID:** REQ-013
- **Title:** Responsive Canvas & HiDPI Handling — 16:9 Maximized, Dual Canvas, Centered, Only Canvases
...
## Description
The **two stacked canvases** (REQ-002) SHALL handle high-DPI displays and responsive sizing without blurriness or input misalignment, while keeping **logical coordinates at 16:9** (e.g., `1280×720` or `1600×900`) for physics consistency and **maximizing the canvas area** centered on screen. **No elements SHALL exist outside `#game-container` except the black "Loading..." screen** — no `<h1>` and no `#instructions` — so `body` can be `padding:0; overflow:hidden; background:#000` and the 16:9 container can truly maximize to `min(95vw, 95vh*16/9)`.

## Rationale
Canvas rendered at CSS size on Retina displays appears blurry if not scaled by `devicePixelRatio`. A 16:9 container that maximizes `min(95vw, 95vh*16/9)` ensures the game uses the largest possible centered rectangle on any viewport (desktop, laptop, tablet landscape) while staying strictly 16:9. Both canvases must share the same backing-store scaling and input mapping. Removing outside elements and body padding ensures the container is not shrunk or offset.
...
2. **Responsive Layout — 16:9 Maximized, Centered, No Outside Elements**:
   - `body` SHALL be `display:flex; align-items:center; justify-content:center; min-height:100vh; margin:0; padding:0; background:#000; overflow:hidden;` during loading (after load background may stay `#000` or `#1a1a1a`, but during loading it MUST be black — see REQ-030). `padding:20px` is **removed** (was 20px in prior draft) because it would prevent true maximization.
   - `#game-container` SHALL be `position:relative; aspect-ratio:16/9; width: min(95vw, calc(95vh * 16/9)); height: min(95vh, calc(95vw * 9/16)); margin:auto;` — this maximizes area without overflow. **No `<h1>` and no `#instructions` outside** SHALL exist to steal space (`document.querySelector('h1')===null`, `document.getElementById('instructions')===null` — see REQ-002/012). Only `#game-container` (+ `#loading-screen` fixed during load) are children of `body`.
   ...
   - Overlays (`#pause-overlay`, `#main-menu-overlay`, `#win-overlay`, `#hotbar`, `#help-overlay`, `#course-menu` inside main menu) SHALL be `position:absolute; inset:0` (or anchored to container edges) with `width:100%; height:100%` bounded to the container, so they never overflow the canvas area (see REQ-029). The wind overlay (`#wind-canvas`) SHALL also be `inset:0` with `z-index:3` and `pointer-events:none`.
   - Scrollable children (`#course-list`, `#help-overlay`) SHALL use `overflow-y:auto` + `overscroll-behavior:contain` with `max-height` constraints so they scroll internally without causing page overflow — never `body` scroll.

3. **Resize Handling**:
   - Listen to `window.resize` and re-apply dpr setup for **all three layers** (both 2D canvases + Three.js wind renderer, debounced 100-200ms) without resetting game state. On resize, redraw bottom background (tile/splash per REQ-030) at new DPR scale **and** update `windRenderer.setSize` / `uResolution` / camera. Hide/re-evaluate `#loading-screen` is not re-shown on resize after initial load.
...
- [ ] `document.querySelector('h1') === null` and `document.getElementById('instructions') === null` and `getComputedStyle(document.body).padding === "0px"` and `getComputedStyle(document.body).overflow === "hidden"` and `getComputedStyle(document.body).backgroundColor === "rgb(0, 0, 0)"` during loading (black). The canvas is centered and maximized (no header/footer offset).
...
- [ ] No canvas/overlay overflows `#game-container`: `game.getBoundingClientRect()`, `wind-canvas.getBoundingClientRect()`, and `main-menu-overlay.getBoundingClientRect()` and `help-overlay.getBoundingClientRect()` and `course-list.getBoundingClientRect()` are contained within `container.getBoundingClientRect()` (no `left<container.left` or `right>container.right`). `course-list` and `help-overlay` are scrollable (`overflow-y:auto`) instead of overflowing.
...
- [ ] While splash is loading, `#loading-screen` is visible with `Loading...` centered on black; after splash loads, it is hidden and does not reappear on resize.

## Dependencies
- REQ-002 (dual canvas setup, 16:9 logical, only canvases)
- REQ-012 (styling, no outside elements)
- REQ-030 (background image rendering, loading screen)
...
- Body `padding:0` is normative now; the previous `padding:20px` would shrink the maximized container and is explicitly removed.

## File Paths
- `src/main.js:10` (setupCanvas for bg+fg, handleResize, getCanvasMousePos, hide loading on splash decode)
- `src/render.js:10` (ctx transform for both)
- `style.css:15` (#game-container 16:9 maximized centering no outside elements, canvas absolute stacked, body black padding 0 overflow hidden, loading screen)
- `index.html:1` (only #game-container + #loading-screen, NO h1, NO #instructions)
