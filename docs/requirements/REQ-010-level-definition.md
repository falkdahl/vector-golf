...
## Description
On start of a **new game**, the game SHALL **procedurally generate `count` levels (holes) where `count` is `3`, `6`, `9` or `18`** with **monotonically increasing difficulty (linear scaling for `6`/`9`/`18`, uniform for `3`)** using a **5-step layered generation pipeline** that combines `Quadratic/Cubic Bézier Curves or Directed A* Pathfinding` for the fairway spine, `Signed Distance Fields (SDFs)` for zone masking, `Simplex/Perlin Noise with Domain Warping` for organic contours, `Cellular Automata (water) + Poisson Disc Sampling (trees)` for hazards/obstacles, and `Physics Raycasting / A* Shot Simulation` for playability validation. Generation is deterministic via a seed and SHALL be performed in `src/levels.js` (e.g., `generateLevels(seed, count=18)` or `generateLevels(seed, 3, {difficulty:'hard'})`). Each generated level SHALL have `tee` on the **left side** and `hole` on the **right side** with **random height**, SHALL define `field` and `obstacles` per progression, **and SHALL define `terrain` zones** (`Green`, `Fairway`, `Rough`, `Out of Bounds`, `Water`) **rendered with fixed colors**: `Green` light green, `Fairway` slightly darker green, `Rough` even darker green, `OB` even darker gray, `Water` blue, and **trees SHALL be circular obstacles**. The static `LEVELS` array, if kept, SHALL be overwritten by the generated `count` at runtime; `LEVELS.length` SHALL be `count` after generation. **For `6`, `9` and `18` hole courses, hole 1 SHALL be `easy` and the last hole SHALL be `hard` scaling linearly; for `3`-hole courses all holes SHALL match the player-selected difficulty `easy`/`medium`/`hard` uniform (see REQ-034).**
...
1. **Generation entry point in `src/levels.js`**:
   - SHALL export `LEVELS: Level[]` (length `count` after generation) and a function `generateLevels(seed?: number, count?: number, options?: {difficulty?: 'easy'|'medium'|'hard'}): Level[]` (or `createLevels`, `generateProceduralLevels`) that (re)creates `count` levels where `count` SHALL be one of `3`, `6`, `9` or `18`. `src/main.js` SHALL call this via `generateCourse` on **new game start** (`startNewGame()`, `startNewGameFromMain()`, `resetGameAfterWin()`, and initial load with no save) before `loadLevel(0)`. The seed MAY be `Date.now()` or `Math.floor(Math.random()*1e9)` for fresh games, or a fixed debug seed if `?seed=` is present; generation MUST be deterministic for a given seed **and** `count`/`difficulty`. **For `6`, `9` and `18` hole courses, difficulty SHALL scale linearly so hole 1 is `easy` and the last hole (`6`, `9` or `18`) is `hard` (see REQ-034 §4 linear mapping); for `3`-hole courses `options.difficulty` SHALL be `'easy'|'medium'|'hard'` selected by the player and all holes SHALL have `level.difficulty.tier === difficulty` uniform.**
   - `LEVELS` after generation SHALL have `length === count` (e.g., `3`, `6`, `9` or `18`), each element as:
     ```js
     {
       id: "hole-1" … "hole-18",
       name: `Hole ${i+1}`,
...
9. **Field & Obstacle Progression With Terrain — Superseded By REQ-034 Difficulty Calculation (Including Sink Slightly Outside Free Edge for Easy/Medium Not Near Green, Flipped/Extra Sources/Sinks Slightly Outside, Bigger Bends, Tighter Fairway):**
   - Level difficulty SHALL be calculated per **REQ-034** from four factors: fairway shape (I/L/V/U/S/Z **with bigger bends on medium/hard and tighter fairway on hard**), field components (with **sink slightly outside free edge not closest to green for easy/medium (20-60px outside), flipped source/sink slightly outside + extra source/sink slightly outside on free edges for hard, extra sink slightly outside free edge not near green for medium**), trees on fairway, water on fairway. The per-hole budgets per tier SHALL be:
     - **Easy:** shape `I` (horizontal/vertical, straight, tiny offset <15px, `W_fairway` baseline 90-140 not tightened), `waterOnFairway===0`, `treesOnFairway∈[1,2]`, `field: sources=1 slightly outside near tee (outside 20-60px, `ex<0||ex>width||ey<0||ey>height`, `dist≤180+OUTSIDE`) + sinks=1 **slightly outside a free canvas edge NOT closest to green** (`sinkEdge !== greenClosestEdge && sinkEdge !== sourceEdge`, outside 20-60px `ex<0||...`, not edge-only, not `dist≤180` to hole) + doublets=1 in fairway (if tree on fairway, doublet in middle of tree ≤2px, applies to all difficulties)`, `vortexes=0` (total 3 components).
     - **Medium:** shape `L/V/U` with **even bigger bends** (`perp offset >45px` vs `>30px` before, `60-100` for L, `50-80` for V, `70-110` same-side for U, total heading 70-220°), `treesOnFairway∈[2,3]`, `waterOnFairway===1`, `field: sources=1 slightly outside near tee (outside 20-60px) + sinks=1 **slightly outside a free edge not closest to green** (`sinkEdge !== greenClosestEdge`, outside 20-60px `ex<0||...`) **plus MAYBE an extra sink slightly outside another remaining free edge also not near green (outside 20-60px)** (60% of medium holes have `sinks=2` both slightly outside free edges not near green) + doublets∈[2,3] + vortexes=1` (total 5-6 or 6-7 with extra sink, at least one doublet in a fairway tree).
     - **Hard:** shape `S/Z` with **even bigger bends** (`perp offsets >55px` opposite sides, `70-110` each, `S/Z` inflection) and **tighter fairway** (`W_fairway` **15-25px smaller** than baseline for easy/medium, `W_fairway_hard = clamp(W_fairway_baseline - (15+rand()*10), 70, 140)`), `treesOnFairway∈[3,5]`, `waterOnFairway∈[1,3]`, `field: **MAY flip** so **sink slightly outside tee side + source slightly outside green side** (sink slightly outside near tee `dist≤180+OUTSIDE` outside 20-60px, source slightly outside near hole `dist≤180+OUTSIDE` outside) **plus extra source and sink slightly outside the two free edges (outside 20-60px)** (so `sources=2, sinks=2` slightly outside when flipped+extra, at least 50% of hard holes SHALL be flipped when sampling 100 hard holes) or if not flipped hard retains sink slightly outside near green (`dist(sink,hole)≤180+OUTSIDE` outside) and **still adds extra source and sink slightly outside free edges (outside)** (so hard always has `sources=2, sinks=2` slightly outside via flipped or extra), `+ doublets∈[3,4] + vortexes∈[1,2]` (total 7-10 with extra outside source/sink, 6-8 without), at least one doublet in tree, remaining interior fairway/rough.
   - The previous monotonic progression table SHALL be considered **superseded** by the tier budgets above; for `6`, `9` and `18` hole courses the generator SHALL pick a tier **linearly scaled so hole 1 is `easy` and the last hole (`6`, `9` or `18`) is `hard`** (e.g., `18`: `1-6 easy, 7-12 medium, 13-18 hard`; `9`: `1-3 easy, 4-6 medium, 7-9 hard`; `6`: `1-2 easy, 3-4 medium, 5-6 hard`; `tier = floor((levelNum-1)/count * 3)` → `0 easy,1 medium,2 hard`) and then generate shape/field/trees/water matching that tier **with bigger bends for medium/hard and tighter fairway for hard**. For `3`-hole courses the generator SHALL create all holes with uniform tier equal to `options.difficulty` (`easy`→ all `I`/all easy budgets, `medium`→ all `L/V/U`, `hard`→ all `S/Z`, see REQ-034). The `count`-hole set SHALL contain the appropriate distribution (e.g., `18` has at least `3` of each tier, `6`/`9` linearly, `3` uniform). See REQ-034 for shape classification with bigger bends, source/sink placement with flipped/extra, and the exact placement rules.
   - Every generated field SHALL still satisfy REQ-003 with the new placement constraints (source **slightly outside** near tee edge (outside 20-60px), **sink slightly outside free edge not near green for easy/medium (outside), sink slightly outside near green edge for hard non-flipped (outside), flipped sink slightly outside near tee + source slightly outside near green for hard flipped (outside)**, doublets interior fairway/rough, at least one doublet in tree when `treesOnFairway≥1`). `strength` still `80→125`.

10. **No hard-coded levels, Course Wrapper (REQ-031)**:
   - `src/levels.js` SHALL NOT contain 18 hand-written objects as the source of truth; instead it SHALL contain the generator and optionally a `STATIC_LEVELS` fallback for tests. `src/main.js` SHALL use the generated `LEVELS` for `currentHoleIndex`, `Hole: N/M` HUD, and `createField` calls. `generateLevels` SHALL support variable `count` `3|6|9|18` plus `options.difficulty` for `3` (`generateLevels(seed, 3, {difficulty:'hard'})` uniform, `6`/`9`/`18` linearly scaled easy→hard) and `LEVELS` after generation SHALL equal `course.holes` for the active course.

## Acceptance Criteria

- [ ] On fresh new game (clear `localStorage`, reload, or call `generateLevels(Date.now())`), `LEVELS.length` equals `count` (`18` by default), `generateLevels(Date.now(),18)` returns 18 with `LEVELS[0].id === "hole-1"` … `LEVELS[17].id === "hole-18"`; additionally `generateLevels(seed,6)` returns 6 (`hole-1`…`hole-6`), `generateLevels(seed,9)` returns 9, `generateLevels(seed,3,{difficulty:'easy'})` returns 3 all `easy`, `generateLevels(seed,3,{difficulty:'hard'})` returns 3 all `hard`. Length matches requested `3|6|9|18`.
...
- [ ] `src/main.js` on new game generates `count` (`3|6|9|18` via `generateCourse`), sets `LEVELS = generateLevels(seed,count,options)`, `currentHoleIndex=0`, `Hole: 1/count` HUD (e.g., `1/6`, `1/9`, `1/3`), and `loadLevel(0)` uses `LEVELS[0].field` and `terrain` to create field and draw terrain zones with correct colors. For `6`/`9`/`18` hole 1 `easy` and last hard linearly; for `3` holes all match selected `easy`/`medium`/`hard`.
