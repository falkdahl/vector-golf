...
## Description
The player SHALL have a supply (inventory) of modifiers at his disposal. Each modifier type (`amplify`, `nullify`, `flip`) SHALL have its own independent counter in the supply. The number of active modifiers of a given type currently placed on the field SHALL NOT be allowed to exceed the current supply count for that type at time of placement. The supply SHALL start with one of each modifier (`{ amplify: 1, nullify: 1, flip: 1 }`) on a new game.

## Rationale
Unlimited placement (REQ-015 §2.5) removes strategic scarcity. An inventory model turns modifiers into a resource to be managed: the player must decide where to spend limited modifiers. Per-type counters give distinct scarcity per effect (e.g., many Amplify but few Flip). Starting with one of each ensures the player can experiment immediately on the first hole without waiting for a reward, while still requiring acquisition via rewards (REQ-021) to expand capacity; until additional rewards are earned, only one of each can be placed. Future acquisition mechanics increment from this baseline.
...
2. **Placement Limit Enforcement** in `src/main.js:placeModifier()` and `src/input.js` / canvas handlers:
   ...
   - Placement SHALL be allowed **iff** `activeCount < supply[selectedModifier]`. If `activeCount >= supply[selectedModifier]`, placement SHALL be rejected: no modifier added to `modifiers`, no `syncModifiersToField()`, selection MAY remain (or optionally clear) but no placement occurs.
...
- [ ] On fresh page load (new game) hotbar shows `Amplify 1`, `Nullify 1`, `Flip 1` (or `x1` badges) for all three slots, all enabled (not grayed).
- [ ] With supply `{amplify:1, nullify:1, flip:1}`, each type can be placed exactly once. Selecting `Amplify` (`1`) and left-clicking canvas places one amplify; `activeCount(amplify) === 1` and placement succeeds; attempting a second amplify is rejected.
- [ ] After programmatically setting `supply = { amplify:2, nullify:1, flip:1 }` (or via future acquisition), hotbar shows `Amplify 2` enabled, others `1` enabled. Selecting Amplify shows orange preview; left-click places one amplify circle; `activeCount(amplify) === 1` and placement succeeds; second amplify also succeeds (`1→2`), third is blocked.
...
- Implementation sketch in `src/main.js`:
  ...
  // init/reset: supply = {amplify:1,nullify:1,flip:1}
  // advanceHole/loadLevel: do NOT reset supply
  // resetGameAfterWin/startNewGameFromMain/endRun/clearProgress: reset supply to {1,1,1}
  ```
...
- Hotbar badge example: `<span class="count">Amplify ×<em data-count="amplify">1</em></span>` updated in `updateHotbarUI()`.
