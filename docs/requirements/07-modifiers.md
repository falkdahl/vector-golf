...
- **Stacking/limits & Auto-arm**: Only one free shot can be armed at a time (`isFreeShotActive` boolean, not counted). Multiple `freeShot` supply just allows multiple future free launches sequentially, **persisting automatically** after each free launch while supply remains (no re-arm needed), otherwise re-arming via `4` when armed was cleared (supply reached `0` or manually toggled off). If at launch init `getAttemptsLeft()<=1` and `supply.freeShot>0` and not already armed, **auto-arm** (`isFreeShotActive=true`) so the last attempt is automatically free. `freeShot` cannot be placed as a circle and does not participate in `canPlace`/drag/remove/area upgrades.
