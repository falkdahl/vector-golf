...
- **Visual feedback — golden glowing effect over ball in Three.js layer + subtle edge glow**: While `isFreeShotActive===true` (armed before launch) **and while the ball is in motion after a free launch** (`freeShotFlightActive` — glow persists through `FLYING` until ball stops, resets, or wins), the Three.js overlay (`#wind-canvas`, `src/windThree.js`) renders a **golden glowing effect centered on `ball.pos`** above the ball. Implementation may be a `THREE.Sprite`/`Mesh`/`Point` with `additive` blending, `color 0xf1c40f` / `0xFFD700`, `opacity 0.55-0.85`, `radius 14-22px` logical (scaled by DPR), soft falloff, pulsation `scale 1.0 + 0.15*sin(time*3)` and `opacity` flicker. Must be in `windThree.js` on `#wind-canvas` (z-index 3-4) so it appears over ball but below HTML overlays; must use `pointer-events:none`, `depthWrite:false`, `transparent:true`. **During any free-shot glow (armed or in-flight) a subtle gold edge glow is also shown** around the canvas: `div#free-shot-edge-glow` `position:absolute; inset:0; border-radius:8px; border:2px solid rgba(241,196,15,0.38); box-shadow: inset 0 0 36px rgba(241,196,15,0.22), inset 0 0 70px rgba(241,196,15,0.12), 0 0 18px rgba(241,196,15,0.28); opacity 1 when active else 0, transition 220-260ms, z-index:4, pointer-events:none`. Canvas 2D fallback (if Three not ready) may draw gold ring `rgba(241,196,15,0.45)` `stroke 2px` around ball, but Three.js gold glow + edge glow is normative. When neither armed nor in free flight, no gold glow is shown.
...
- [ ] `getWindAt` inside amplify `5×`, inside nullify `{0,0}` (physics preserves velocity), inside flip `*-1`; stacking as specified; effective radius used for spatial; `freeShot` does **not** affect `getWindAt`.
...
- [ ] Free Shot active: golden glowing effect over ball visible in Three.js layer (`#wind-canvas`) while `isFreeShotActive===true` **and while ball is in motion after a free launch** (`freeShotFlightActive`, gold `0xf1c40f`, additive, pulsating, `pointer-events:none`) plus subtle gold edge glow around canvas (`#free-shot-edge-glow` border+shadow); inactive and not in free flight shows no glow. Launch with active does **not** increment `holeAttempts`/`totalAttempts` nor decrement `attemptsLeft`; after launch `supply.freeShot` decreases by exactly `1` and armed clears but glow persists through flight until ball stops/resets/wins; normal launches still cost `1` attempt.
